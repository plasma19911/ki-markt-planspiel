...
function drawChart(rows){
 const c=$("chart"),ctx=c.getContext("2d"),w=c.width,h=c.height;ctx.clearRect(0,0,w,h);ctx.strokeStyle="#20324a";ctx.lineWidth=1;
 ...
 const vals=rows.map(x=>Number(x.equity));let min=Math.min(...vals),max=Math.max(...vals);if(min===max){min-=1;max+=1}const pad=(max-min)*.12;min-=pad;max+=pad;ctx.strokeStyle="#7aabff";ctx.lineWidth=3;ctx.beginPath();
 vals.forEach((v,i)=>{const x=12+i/(vals.length-1)*(w-24),y=h-15-(v-min)/(max-min)*(h-32);i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke();ctx.fillStyle="#cbd9ec";ctx.font="12px sans-serif";ctx.fillText(`${fmt(max)} – ${fmt(min)} ${currency}`,12,15);
}

function pieColor(i){const hue=(i*137.508+205)%360;return `hsl(${hue} 68% 62%)`}
function drawAllocation(positions,cash){
 const c=$("allocationChart"),ctx=c.getContext("2d"),w=c.width,h=c.height;ctx.clearRect(0,0,w,h);
 const items=(positions||[]).map(p=>({name:p.symbol,detail:p.name||p.symbol,value:Number(p.invested)*(Number(p.last_price)/Number(p.entry_price))})).filter(x=>x.value>0);if(Number(cash)>0)items.push({name:"CASH",detail:"Nicht investiert",value:Number(cash)});
 const total=items.reduce((s,x)=>s+x.value,0);if(total<=0){ctx.fillStyle="#8fa3bd";ctx.font="16px sans-serif";ctx.textAlign="center";ctx.fillText("Noch keine Depotaufteilung.",w/2,h/2);ctx.textAlign="start";$("allocationLegend").innerHTML="";return}
 const cx=w/2,cy=h/2-5,r=Math.min(w,h)*.33,inner=r*.52;let start=-Math.PI/2;items.forEach((item,i)=>{const a=item.value/total*Math.PI*2,end=start+a;ctx.beginPath();ctx.moveTo(cx,cy);ctx.arc(cx,cy,r,start,end);ctx.closePath();ctx.fillStyle=pieColor(i);ctx.fill();start=end});
 ctx.globalCompositeOperation="destination-out";ctx.beginPath();ctx.arc(cx,cy,inner,0,Math.PI*2);ctx.fill();ctx.globalCompositeOperation="source-over";ctx.textAlign="center";ctx.fillStyle="#eef5ff";ctx.font="700 24px sans-serif";ctx.fillText(money(total),cx,cy-2);ctx.fillStyle="#8fa3bd";ctx.font="12px sans-serif";ctx.fillText("aktuelle Aufteilung",cx,cy+20);ctx.textAlign="start";
 $("allocationLegend").innerHTML=items.map((item,i)=>`<div class="legendItem"><span class="legendDot" style="background:${pieColor(i)}"></span><span class="legendName"><b>${item.name}</b> · ${fmt(item.value/total*100,1)}%</span><span class="legendValue">${money(item.value)}</span></div>`).join("");
}
...
function historyTime(h){
 if(h.action==="HALTEN"){
  const n=Math.max(1,Number(h.event_count)||1),start=Number(h.start_scan),end=Number(h.end_scan);
  const scans=Number.isFinite(start)&&start>0?(end>start?`Scan ${start}–${end}`:`Scan ${start}`):(n>1?`Scans ${n}× zusammengefasst`:`HALTEN`);
  return `${scans} · von ${dt(h.ts)}<br>bis ${dt(h.end_ts||h.ts)}`;
 }
 return dt(h.ts);
}
...
async function load(){
 try{
  ...
  if(!initialized){
   $("startCapital").value=c.start_capital||100;$("currency").value=c.currency||"EUR";$("riskMode").value=c.risk_mode||"offensiv";$("includeEtfs").checked=Boolean(c.include_etfs);$("includeLeverage").checked=Boolean(c.include_leverage);$("aiEnabled").checked=Boolean(c.ai_enabled);
   const oldModel=!c.running&&((Math.abs(Number(c.fee_fixed)-3.7)<.001&&Math.abs(Number(c.fee_percent)-.08)<.001)||(Number(c.fee_fixed??1)===1&&Number(c.fee_percent??0)===0));
   $("feeFixed").value=Number(oldModel?1:(c.fee_fixed??1)).toFixed(2);$("feePercent").value=Number(oldModel?0:(c.fee_percent??0)).toFixed(2);initialized=true;
  }
