...
function renderLearning(s={}){
 const section=ensureSection();if(!section)return;const meta=$('newsLearningMeta'),body=$('newsLearningBody'),l=s.newsLearning,sm=l?.summary;if(!l||!sm){meta.textContent='Noch keine News-Wirkungshistorie. Nach neuen Meldungen beginnt die Auswertung.';body.innerHTML='<tr><td colspan="5">Lernphase noch nicht gestartet.</td></tr>';return}const lag=Number.isFinite(Number(sm.reactionLag?.avgDirectionalMinutes))?` · Ø erste Reaktion ${Math.round(Number(sm.reactionLag.avgDirectionalMinutes))} Min.`:'';meta.textContent=`${sm.evaluatedEvents||0} Meldungen ausgewertet · ${sm.pendingEvents||0} offen · ${sm.totalEvents||0} gespeichert · regionale Benchmarks${lag} · ${sm.notice||''}`;const html=rows(sm.topSources||[],'Quelle: ')+rows(sm.topTypes||[],'Typ: ');body.innerHTML=html||'<tr><td colspan="5">Noch zu wenige abgeschlossene Auswertungen für ein Ranking.</td></tr>'
}
