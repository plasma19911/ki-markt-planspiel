...
function installLiveNewsStyle(){
 ...
 #liveStockNews .liveNewsImpact small,#liveStockNews .liveNewsImpact b,#liveStockNews .liveNewsImpact em{display:block;font-style:normal}#liveStockNews .liveNewsImpact small{font-size:7px;color:#789bb2}#liveStockNews .liveNewsImpact b{font-size:13px;color:#eef8ff;margin:1px 0}#liveStockNews .liveNewsImpact em{font-size:7.5px}
...
function feedRow(x){
 const imp=Number(x.importance||0),cls=imp>=88?'veryhigh':imp>=72?'high':'',direction=x.directionLabel||'RICHTUNG OFFEN',effect=Number(x.expectedValue||0),basis=String(x.materialityBasis||'ANNAHME').toLowerCase(),samples=Number(x.learnedSamples||0),sources=(x.sources||[]).map(s=>s.name).filter(Boolean).slice(0,2).join(', ')||'News-Radar',stocks=(x.affected||[]).map(a=>`<button type="button" class="liveNewsStock" data-symbol="${esc(a.symbol)}" data-name="${esc(a.name||a.symbol)}" title="Live-Chart öffnen"><b>${esc(a.symbol)}</b><small>${esc(a.name||a.symbol)}</small></button>`).join('');
 ...
 return `<article class="liveNewsItem"><div class="liveNewsImpact ${cls}" title="Wichtigkeit ist nicht gleich Richtung oder Sicherheit"><small>WICHTIGKEIT</small><b>${imp}/100</b><em>${esc(direction)}</em></div><div>${h}<div class="liveNewsSub"><span>${esc(eventLabel(x.eventType))}</span><span>Wirkungsimpuls ${effect>=0?'+':''}${fmt(effect,1)}/100</span><span>Stärke ${esc(basis)}${samples?` · ${samples} Fälle`:''}</span><span>${esc(ageText(x.publishedAt))}</span><span>${esc(sources)}</span></div></div><div class="liveNewsStocks">${stocks}</div></article>`
}
async function loadFeed(){
 ...
 try{const r=await fetch(`/api/news-feed?t=${Date.now()}`,{cache:'no-store',headers:{'cache-control':'no-cache'}});if(!r.ok)throw new Error(`HTTP ${r.status}`);const j=await r.json(),items=Array.isArray(j.items)?j.items:[];list.innerHTML=items.length?items.map(feedRow).join(''):'<div class="liveNewsEmpty">In den letzten 2 Stunden gab es keine eindeutig aktienbezogene wichtige neue Meldung. Ältere Radar-Meldungen bleiben getrennt sichtbar und werden weiter ausgewertet.</div>';const t=Date.parse(j.generatedAt||'');fresh.textContent=Number.isFinite(t)?`Stand ${new Date(t).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'})}`:'Live';meta.textContent=`Wichtigkeit = Firmenbezug × typische Ereignisstärke. Richtung und Wirkungsimpuls stehen getrennt. ${items.length} angezeigt · ${j.totalDetected||0} passend · ${j.totalCollectedBeforeAgeFilter||0} geprüft · Aktualisierung jede Minute.`}catch(e){fresh.textContent='Feed gestört';meta.textContent=`Live-News derzeit nicht lesbar: ${e.message}`;list.innerHTML='<div class="liveNewsEmpty">News-Feed wird beim nächsten Minuten-Takt erneut geladen.</div>'}
}
...
function ensureSection(){
 if($('newsLearning'))return $('newsLearning');const news=$('news');if(!news)return null;const s=document.createElement('section');s.id='newsLearning';s.className='card';s.innerHTML='<div class="cardTitle"><h2>Gelernte News-Wirkung</h2><span class="tag">Quelle × Ereignis</span></div><div id="newsLearningMeta" class="muted">Lernphase startet mit neuen Meldungen.</div><div class="tableWrap"><table><thead><tr><th>Quelle / Typ</th><th>Auswertungen</th><th>Beobachtet</th><th>Ø Marktreaktion</th><th>Quellenwert</th></tr></thead><tbody id="newsLearningBody"></tbody></table></div><div class="notice">„Beobachtet“ zeigt echte Treffer je ausgewerteter Meldung. Unter 3 Auswertungen wird bewusst keine Prozentquote behauptet; der interne geglättete Lernwert bleibt davon getrennt. Gemessen wird relativ zum passenden Regionalmarkt.</div>';news.insertAdjacentElement('afterend',s);return s;
}
function rows(list,prefix=''){if(!list?.length)return '';return list.map(x=>{const samples=Number(x.samples||0),wins=Number(x.wins||0),observed=Number.isFinite(Number(x.observedHitRate))?Number(x.observedHitRate):samples?wins/samples:null,hit=samples<3?`WARMUP · ${wins}/${samples}`:`${fmt(observed*100,0)}% · ${wins}/${samples}`,value=samples<12?`vorläufig · ${fmt(x.reliabilityScore,0)}/100`:`${fmt(x.reliabilityScore,0)}/100`;return`<tr><td><b>${esc(prefix+x.key)}</b></td><td>${samples}</td><td>${hit}</td><td class="${Number(x.avgAlignedPct)>=0?'good':'bad'}">${Number(x.avgAlignedPct)>=0?'+':''}${fmt(x.avgAlignedPct,2)}%</td><td><b>${value}</b></td></tr>`}).join('')}
function renderLearning(s={}){
 const section=ensureSection();if(!section)return;const meta=$('newsLearningMeta'),body=$('newsLearningBody'),l=s.newsLearning,sm=l?.summary;if(!l||!sm){meta.textContent='Noch keine News-Wirkungshistorie. Nach neuen Meldungen beginnt die Auswertung.';body.innerHTML='<tr><td colspan="5">Lernphase noch nicht gestartet.</td></tr>';return}const lag=Number.isFinite(Number(sm.reactionLag?.avgDirectionalMinutes))?` · Ø erste Reaktion ${Math.round(Number(sm.reactionLag.avgDirectionalMinutes))} Min.`:'',total=Number(sm.totalEvents??s.newsLearningTotal??sm.pendingEvents??0),benchmark=l.benchmark==='REGIONAL'?'regionale Benchmarks':'Umstellung auf regionale Benchmarks läuft';meta.textContent=`Lernspeicher ${total} Ereignisse · ${sm.evaluatedEvents||0} mindestens einmal gemessen · ${sm.completedEvents||0} vollständig · ${sm.pendingEvents||0} offen · ${benchmark}${lag}.`;const html=rows(sm.topSources||[],'Quelle: ')+rows(sm.topTypes||[],'Typ: ');body.innerHTML=html||'<tr><td colspan="5">Noch zu wenige abgeschlossene Auswertungen für ein Ranking.</td></tr>'
}
document.addEventListener('click',e=>{const b=e.target.closest?.('.liveNewsStock');if(b)openChart(b.dataset.symbol,b.dataset.name)});
document.addEventListener('planspiel:status',e=>renderLearning(e.detail||{}));
document.addEventListener('DOMContentLoaded',()=>{ensureFeed();ensureChartModal();loadFeed()});
if(document.readyState!=='loading'){ensureFeed();ensureChartModal();loadFeed()}
setInterval(()=>{if(!document.hidden)loadFeed()},REFRESH_MS);
