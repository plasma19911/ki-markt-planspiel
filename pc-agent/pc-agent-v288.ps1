...
$AgentVersion='1.2.5-v31.7.31-aligned-quote-time'
...
while($true){
  try{$now=Get-BerlinNow;$session=Get-Session $now;if(-not $session.trading -or $session.minute -lt 440 -or $session.minute -ge 1385){try{Invoke-AgentPost '/api/agent/heartbeat' (Get-Metrics $session.phase)|Out-Null}catch{};Start-Sleep -Seconds 60;continue};$metrics=Get-Metrics $session.phase
