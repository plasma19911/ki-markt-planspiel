...
$agentSource=Join-Path $Source 'pc-agent.ps1'
if(-not(Test-Path $agentSource)){throw 'pc-agent.ps1 fehlt im Installationspaket.'}
...
Write-Host '';Write-Host 'KI-Markt-Agent V31.7.34 wurde installiert.' -ForegroundColor Green;Write-Host "Ordner: $Root";Write-Host 'Scanner: kompletter Aktien-Master rollierend -> Top 400 -> Deep 240 -> Final 60.' -ForegroundColor Green;Write-Host 'Finalisten und der bestaetigte Handelsscan werden jede Minute aktualisiert.';Write-Host 'Cloudflare bleibt Finalvalidierung und Frische-Fallback.';Write-Host ''
