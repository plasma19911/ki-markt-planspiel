...
# Autostart bewusst pro Benutzer statt Windows-Taskplaner: funktioniert ohne Administratorrechte.
...
# Einen eventuell unvollständig angelegten alten Task still entfernen; Fehler werden bewusst ignoriert.
...
Write-Host 'Autostart: Windows-Benutzerautostart (HKCU), KEINE Administratorrechte nötig.' -ForegroundColor Green
Write-Host ''
try{& (Join-Path $Source 'start-agent.ps1') -Root $Root}catch{Write-Host "Agent konnte noch nicht gestartet werden: $($_.Exception.Message)" -ForegroundColor Yellow}
Write-Host ''
Write-Host 'WICHTIG: Als naechstes CLOUDFLARE-SECRET-EINRICHTEN.cmd aus diesem Ordner starten.' -ForegroundColor Yellow
Write-Host 'Der bisherige Fehler "Zugriff verweigert" vom Taskplaner ist mit dieser Version behoben.'
