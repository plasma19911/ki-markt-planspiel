...
function Get-RotatingSlice($Rows,[int]$Start,[int]$Count){$a=@($Rows);if($a.Count -eq 0 -or $Count -le 0){return @()};$out=New-Object System.Collections.Generic.List[object];for($i=0;$i -lt [Math]::Min($Count,$a.Count);$i++){$out.Add($a[(($Start+$i)%$a.Count)])};return @($out)}
function Add-UniqueRows($Target,$Seen,$Rows,[int]$Limit=999999){foreach($r in @($Rows)){if($Target.Count -ge $Limit){break};$s=([string]$r.symbol).ToUpperInvariant().Trim();if(-not $s -or $Seen.ContainsKey($s)){continue};$Seen[$s]=$true;$Target.Add($r)}}
function Split-Batches($Rows,[int]$Size){$a=@($Rows);$out=New-Object System.Collections.Generic.List[object];for($i=0;$i -lt $a.Count;$i+=$Size){$end=[Math]::Min($a.Count-1,$i+$Size-1);$out.Add([pscustomobject]@{rows=@($a[$i..$end])})};return @($out)}
function SparkUrl([string]$Host,$BatchRows){$symbols=(@($BatchRows) | ForEach-Object{[string]$_.symbol}) -join ',';return "https://$Host/v7/finance/spark?symbols=$(UrlEncode $symbols)&range=1d&interval=5m&indicators=close&includePrePost=false"}
function Read-SparkBatch($BatchRows,[string]$Text){
  $lookup=@{};foreach($r in @($BatchRows)){$lookup[([string]$r.symbol).ToUpperInvariant()]=$r}
  try{$j=$Text | ConvertFrom-Json}catch{return @()};$rows=New-Object System.Collections.Generic.List[object]
  foreach($item in @($j.spark.result)){
    ...
    $cl=New-Object System.Collections.Generic.List[double];foreach($v in @($res.indicators.quote[0].close)){if($null -eq $v){continue};$n=Num $v -1;if($n -gt 0){$cl.Add($n)}};if($cl.Count -lt 5){continue}
    $last=$cl[$cl.Count-1];$prev=Num $meta.previousClose $cl[0];if($prev -le 0){$prev=$cl[0]};$m5=($last/$cl[$cl.Count-2]-1)*100;$m20=($last/$cl[$cl.Count-5]-1)*100;$prev5=if($cl.Count -ge 3){($cl[$cl.Count-2]/$cl[$cl.Count-3]-1)*100}else{0};$accel=$m5-$prev5;$session=if($prev -gt 0){($last/$prev-1)*100}else{0};$marketTime=Num $meta.regularMarketTime 0
...
function Wait-HttpTasks($TaskRows){$tasks=@($TaskRows | ForEach-Object{$_.task});if(-not $tasks.Count){return};try{[System.Threading.Tasks.Task]::WaitAll([System.Threading.Tasks.Task[]]$tasks,20000) | Out-Null}catch{}}
function Invoke-SparkWave($Batches,[int]$Parallel){
  $allRows=New-Object System.Collections.Generic.List[object];$fail=0;$throttle=0;$requests=0;$a=@($Batches)
  for($offset=0;$offset -lt $a.Count;$offset+=$Parallel){$end=[Math]::Min($a.Count-1,$offset+$Parallel-1);$wave=@($a[$offset..$end]);$tasks=New-Object System.Collections.Generic.List[object];foreach($b in $wave){$tasks.Add([pscustomobject]@{batch=$b;task=$http.GetAsync((SparkUrl 'query1.finance.yahoo.com' $b.rows))})};Wait-HttpTasks @($tasks);$retry=New-Object System.Collections.Generic.List[object]
    foreach($x in @($tasks)){$requests++;try{if(-not $x.task.IsCompleted -or $x.task.IsFaulted){$retry.Add($x);continue};$resp=$x.task.Result;if($resp.IsSuccessStatusCode){$txt=$resp.Content.ReadAsStringAsync().Result;foreach($r in @(Read-SparkBatch $x.batch.rows $txt)){$allRows.Add($r)}}else{if([int]$resp.StatusCode -in @(401,403,429)){$throttle++};$retry.Add($x)}}catch{$retry.Add($x)}}
    if($retry.Count){$rTasks=New-Object System.Collections.Generic.List[object];foreach($x in @($retry)){$rTasks.Add([pscustomobject]@{batch=$x.batch;task=$http.GetAsync((SparkUrl 'query2.finance.yahoo.com' $x.batch.rows))})};Wait-HttpTasks @($rTasks);foreach($x in @($rTasks)){$requests++;try{if(-not $x.task.IsCompleted -or $x.task.IsFaulted){$fail++;continue};$resp=$x.task.Result;if($resp.IsSuccessStatusCode){$txt=$resp.Content.ReadAsStringAsync().Result;foreach($r in @(Read-SparkBatch $x.batch.rows $txt)){$allRows.Add($r)}}else{$fail++;if([int]$resp.StatusCode -in @(401,403,429)){$throttle++}}}catch{$fail++}}}
  };return [pscustomobject]@{rows=@($allRows);failures=$fail;throttles=$throttle;requests=$requests}
}
function Select-UploadRows($Rows){$all=@($Rows);$picked=New-Object System.Collections.Generic.List[object];$seen=@{};$dips=@($all | Where-Object{$_.dipDiscovery} | Sort-Object dipRankScore,accelerationPct,wideScore -Descending | Select-Object -First 400);Add-UniqueRows $picked $seen $dips $SendLimit;$shocks=@($all | Sort-Object shockScore -Descending | Select-Object -First 240);Add-UniqueRows $picked $seen $shocks $SendLimit;$momentum=@($all | Sort-Object wideScore,accelerationPct,m5Pct -Descending | Select-Object -First 400);Add-UniqueRows $picked $seen $momentum $SendLimit;return @($picked | Select-Object -First $SendLimit | ForEach-Object{[pscustomobject]@{symbol=$_.symbol;wideScore=$_.wideScore;m5Pct=$_.m5Pct;m20Pct=$_.m20Pct;accelerationPct=$_.accelerationPct;sessionPct=$_.sessionPct;last=$_.last;observedAt=$_.observedAt;source=$_.source}})}
...
while($true){
  try{Invoke-RadarMinute}catch{$msg=$_.Exception.Message;Write-RadarLog "FEHLER: $msg";try{Send-StartupProbe $msg}catch{};try{$s=Load-State;$s.lastError=$msg;$s.backoffUntilUtc=(Get-Date).ToUniversalTime().AddMinutes(2).ToString('o');Save-State $s}catch{};Start-Sleep -Seconds 8}
