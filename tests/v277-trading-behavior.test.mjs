...
// 1) One-minute scanner cadence must mature confirmation instead of resetting the timer forever.
{
 ...
 const second=enforceTradingBehaviorV277(buyPlan(),state,st,now+1*60000).plan.actions[0];
 assert.equal(second.action,'HOLD','one minute is intentionally not enough');
 const third=enforceTradingBehaviorV277(buyPlan(),state,st,now+2*60000).plan.actions[0];
 assert.equal(third.action,'BUY','two minutes of continuous healthy one-minute scans must mature the entry');
}

// 2) If price runs >0.8% during confirmation, chase is rejected and a fresh baseline is required.
{
 const st=storage(),state={config:{cash:10000},positions:[],candidates:[baseCandidate]};
 enforceTradingBehaviorV277(buyPlan(),state,st,now);
 const jumped={...baseCandidate,price:101};
 const a=enforceTradingBehaviorV277(buyPlan(),{...state,candidates:[jumped]},st,now+2*60000).plan.actions[0];
 assert.equal(a.action,'HOLD');assert.match(a.reason,/Nicht hinterherkaufen/i);
 const stillStable=enforceTradingBehaviorV277(buyPlan(),{...state,candidates:[jumped]},st,now+3*60000).plan.actions[0];
 assert.equal(stillStable.action,'HOLD','fresh post-jump baseline must also mature');
}

// 3) Exceptional clean strength may enter immediately, but only inside anti-chase bounds.
{
...
// 4) OCDOL-style fast impulse must be rechecked instead of chased.
{
...
// 5) When portfolio is already >85% invested, mediocre rest-cash BUY is rejected.
{
...
// 6) A PROFIT EXIT that would realize only cents/small euros stays HOLD without a real winner break.
{
...
// 7) Candidate fx_rate=0 must not erase a held position's last verified FX during profit evaluation.
{
 const st=storage(),p={symbol:'FX.NS',invested:500,entry_fee:1,entry_price:1000,last_price:1006,entry_fx:.01,last_fx:.01,currency:'INR',zero_quantity:50,opened_at:'2026-08-20T11:00:00Z'},c={symbol:'FX.NS',price:1006,fx_rate:0,intraday5m:.02,intraday20m:.04,momentumAcceleration5:0,sellerShare:48,newsScore:0,eventRisk:'NONE'};
 const plan={summary:'FINAL-CONTROLLER V27.6',actions:[{symbol:'FX.NS',action:'SELL',confidence:.73,allocation_pct:0,reason:'FINAL-CONTROLLER PROFIT EXIT: Gewinnerstruktur angeblich gebrochen.'}]};
 const a=enforceTradingBehaviorV277(plan,{config:{cash:1000,slippage_percent:.10},positions:[p],candidates:[c]},st,now).plan.actions[0];
 assert.equal(a.action,'HOLD','held last_fx must be used when current candidate FX is zero');assert.match(a.reason,/PROFIT-PATIENCE V27\.7/);
}

// 8) A meaningful profitable exit is not blocked.
{
