...

{
 ...
 assert.match(out.plan.actions[0].reason,/LOSS-SELL-INVARIANT V27\.5/);
}
...

{
 ...
 assert.equal(out.plan.actions[0].action,'HOLD');
 assert.match(out.plan.actions[0].reason,/ORDER-ECONOMICS V27\.5/);
}

{
 ...
 assert.match(out.plan.actions[0].reason,/V27\.5/,'Versionslabel muss live auf V27.5 normalisiert werden');
 assert.match(out.plan.summary,/V27\.5/);
}

console.log('V27.3/V27.4/V27.5 loss sell invariant regression tests: OK');
