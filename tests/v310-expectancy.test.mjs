...
{
 const state={cash:5000,positions:[],candidates:[{symbol:'TEST.DE',score:72}],history:[{symbol:'TEST.DE',action:'VERKAUF',ts:new Date(now-20*60000).toISOString(),score:70}]};
 const plan={actions:[{symbol:'TEST.DE',action:'BUY',allocation_pct:20,reason:'buy'}],summary:'x'};
 const out=enforceExpectancyCoreV310(plan,state,now);
 assert.equal(out.plan.actions[0].action,'HOLD','persistierte deutsche VERKAUF/ts-Historie muss die Reentry-Sperre aktivieren');
}
{
