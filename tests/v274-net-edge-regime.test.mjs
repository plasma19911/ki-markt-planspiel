...
const buyPlan=(reason,allocation=30)=>({summary:'FINAL-CONTROLLER V27.4',actions:[{symbol:'TEST.DE',action:'BUY',confidence:.72,allocation_pct:allocation,reason}]});
...
{
 const reason='FINAL-CONTROLLER V27.4 BUY EARLY_BREAKOUT: Qualität 0.70 · Setup-Kalibrierung NORMAL_ENTRY: E[Move] +0.050% · Trefferbasis 56.0% · n=12';
 ...
 assert.match(out.plan.actions[0].reason,/NET-EDGE V27\.5/);
}

{
 const reason='FINAL-CONTROLLER V27.4 BUY EARLY_BREAKOUT: Qualität 0.70 · Setup-Kalibrierung NORMAL_ENTRY: E[Move] +0.050% · Trefferbasis 56.0% · n=2';
 const out=enforceLossSellInvariant(buyPlan(reason),state(candidate()));
...
{
 const reason='FINAL-CONTROLLER V27.4 BUY EARLY_BREAKOUT: Qualität 0.72 · Setup-Kalibrierung NORMAL_ENTRY: E[Move] +0.800% · Trefferbasis 62.0% · n=12';
 ...
 assert.equal(out.plan.actions[0].action,'HOLD');
 assert.match(out.plan.actions[0].reason,/MARKET-REGIME V27\.5/);
}

{
 const reason='FINAL-CONTROLLER V27.4 BUY EARLY_BREAKOUT: Qualität 0.82 · Setup-Kalibrierung CONFIRMED_BREAKOUT: E[Move] +0.800% · Trefferbasis 66.0% · n=12';
 ...
 assert.match(out.plan.actions[0].reason,/V27\.5/);
}

{
 const reason='FINAL-CONTROLLER V27.4 BUY EARLY_BREAKOUT: Qualität 0.76 · Setup-Kalibrierung NORMAL_ENTRY: E[Move] +0.650% · Trefferbasis 61.0% · n=12';
 ...
 assert.equal(out.regimeCaps,1);
}

console.log('V27.4/V27.5 net edge / market regime regression tests: OK');
