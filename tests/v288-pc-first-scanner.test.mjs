...
assert.match(src,/scorePipeline:'ALIGNED PRICE\+TIME → ≤8M → TOP400 → DEEP240 → FINAL60 → CLOUDFLARE36 \+ POSITIONS'/);
assert.match(src,/pcQuoteAge/,'PC quote age must be recalculated from the market timestamp');
assert.match(src,/freshCandidates\.length<12/,'insufficient fresh PC data must leave the full Cloudflare fallback active');
