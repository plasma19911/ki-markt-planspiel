...
base="const tradable=coarse.filter(x=>!x.benchmark),freshTradable=tradable.filter(x=>x.fresh),selected=selectBalancedDeepCandidates(freshTradable,DEEP_LIMIT,MOMENTUM_BREAKOUT_RESERVE)"
bad=base+"for(const sym of heldSymbols)"
good=base+";for(const sym of heldSymbols)"
if bad in t:
    t=t.replace(bad,good)
elif old in t:
    t=t.replace(old,base+";")
elif base+";" not in t:
    raise SystemExit('deep selection anchor unresolved')

p.write_text(t,encoding='utf-8')
print('balanced deep selection present and statement-separated')
