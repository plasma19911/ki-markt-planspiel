...
let rows=listOrderApprovals(storage);assert.equal(rows.length,2);const buy=rows.find(x=>x.symbol==='AAA'),sell=rows.find(x=>x.symbol==='BBB');assert.equal(buy.status,'PENDING');assert.equal(buy.estimatedNotional,200);assert.equal(buy.brokerTarget,'finanzen.net ZERO');assert.equal(sell.action,'SELL');
const caps=orderApprovalCapabilities({});assert.equal(caps.enabled,false);assert.equal(caps.brokerDispatchEnabled,false);
...
const approved=approveOrderApproval(storage,buy.id,'tester@example.test');assert.equal(approved.ok,true);assert.equal(approved.brokerSent,false);assert.equal(approved.order.status,'APPROVED_LOCAL');assert.equal(approved.order.dispatchState,'AWAITING_OFFICIAL_CONNECTOR');
...
console.log(JSON.stringify({ok:true,pending:listOrderApprovals(storage).length,approvedBrokerSent:approved.brokerSent,authFailClosed:auth.status},null,2));
