...
PRIMARY_EXCHANGES = {
    "us": {"NMS","NYQ","NGM","NCM","ASE","PCX"}, "ca": {"TOR","VAN","NEO"}, "gb": {"LSE"},
    "de": {"GER"}, "fr": {"PAR"}, "ch": {"EBS"}, "nl": {"AMS"}, "se": {"STO"}, "dk": {"CPH"},
    "no": {"OSL"}, "fi": {"HEL"}, "it": {"MIL"}, "es": {"MCE"}, "be": {"BRU"}, "at": {"VIE"},
    "jp": {"JPX","TYO"}, "hk": {"HKG"}, "cn": {"SHH","SHZ"}, "tw": {"TAI","TWO"}, "kr": {"KSC","KOE"},
    "in": {"NSI","BSE"}, "au": {"ASX"}, "sg": {"SES"}, "br": {"SAO"}, "mx": {"MEX"}, "za": {"JNB"},
}
LISTING_WORDS = {
    "INC","INCORPORATED","CORP","CORPORATION","CO","COMPANY","LTD","LIMITED","PLC","AG","SE","NV","SA","SPA",
    "HOLDING","HOLDINGS","GROUP","ORD","ORDINARY","SHARE","SHARES","SHS","REGISTERED","REG","R","ADR","GDR","CDR",
    "DRN","BDR","ED","HEDGED","HEDGE","ADS","UNIT","UNITS","STOCK","CLASS","CL","SERIES","THE","AND","A","B","C","D","I","II","III"
}
GENERIC_WORDS = {"UNITED","GLOBAL","INTERNATIONAL","TECHNOLOGY","TECHNOLOGIES","INDUSTRIES","INDUSTRIAL","ENERGY","SYSTEMS","FINANCIAL","SERVICES","HOLDING","HOLDINGS","GROUP","COMPANY"}
...
def company_key(name,symbol):
    ...
    return " ".join(toks[:7]) if toks else re.sub(r"[^A-Z0-9]","",symbol.split(".")[0])

def alpha_root(symbol):
    return re.sub(r"[^A-Z]","",str(symbol or "").split('.')[0].upper())
...
def fuzzy_same_company(a,b):
    ...
    # Cross-listing market caps should be nearly identical after FX conversion.
    if abs(ca-cb)/max(ca,cb)>0.035:return False
    ta=set(meaningful_tokens(a.get("name")));tb=set(meaningful_tokens(b.get("name")))
    distinct_a={x for x in ta if x not in GENERIC_WORDS and len(x)>=5};distinct_b={x for x in tb if x not in GENERIC_WORDS and len(x)>=5}
    if distinct_a & distinct_b:return True
    ra,rb=alpha_root(a.get("symbol")),alpha_root(b.get("symbol"))
    common=0
    for x,y in zip(ra,rb):
        if x!=y:break
        common+=1
    if common>=4:return True
    # Fuer kurze internationale Symbolvarianten reicht ein 3er-Stamm nur zusammen mit Namensaehnlichkeit.
    union=ta|tb;inter=ta&tb
    j=len(inter)/len(union) if union else 0
    return common>=3 and j>=0.20

def second_pass_dedupe(items):
    """Fasst verbliebene Abkuerzungs-/Namensvarianten anhand Cap+Name/Ticker zusammen."""
    rows=sorted(items,key=lambda x:float(x.get("marketCapUSD") or 0),reverse=True)
    out=[];collapsed=0
    for item in rows:
        match=None
        # Da nach Cap sortiert, genuegt ein lokales Fenster; jenseits 3.5% ist kein Cross-Listing mehr plausibel.
        for old in reversed(out[-80:]):
            oc=float(old.get("marketCapUSD") or 0);ic=float(item.get("marketCapUSD") or 0)
            if oc and ic and abs(oc-ic)/max(oc,ic)>0.035:continue
            if fuzzy_same_company(item,old):match=old;break
        if match is None:out.append(item)
        else:
            collapsed+=1
            if representative_score(item)>representative_score(match):
                idx=out.index(match);out[idx]=item
...
def main():
    ...
    # Broad passes provide enough raw names even after aggressive company-level dedupe.
    ...
    for q,symbol,mcap,currency in raw_items:
        ...
        musd=mcap*rate;name=q.get("longName") or q.get("shortName") or q.get("displayName") or symbol
        item={"symbol":symbol,"name":name,"marketCap":mcap,"marketCapUSD":musd,"region":q.get("region"),"exchange":q.get("exchange"),"currency":currency,"sector":q.get("sector") or q.get("sectorDisp"),"industry":q.get("industry") or q.get("industryDisp"),"avgVolume":scalar(q.get("averageDailyVolume3Month"),scalar(q.get("averageDailyVolume10Day"),0))}
    ...
    unique,fuzzy_collapsed=second_pass_dedupe(list(by_company.values()))
    top=sorted(unique,key=lambda x:x["marketCapUSD"],reverse=True)[:500]
    if len(top)<450:raise RuntimeError(f"Only {len(top)} unique FX-normalized companies found; refusing overwrite. Failures: {failures[:5]}")

    payload={"generated_at":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"source":"yfinance Yahoo EquityQuery; global region aggregation; FX-normalized representative-listing market caps; exact+fuzzy cross-listing company dedupe","count":len(top),"unique_companies":len(top),"raw_unique_symbols":len(by_symbol),"duplicate_listings_collapsed":exact_collapsed+fuzzy_collapsed,"exact_duplicates_collapsed":exact_collapsed,"fuzzy_duplicates_collapsed":fuzzy_collapsed,"equities":top}
    OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
    print(f"Wrote {len(top)} unique companies; collapsed exact={exact_collapsed}, fuzzy={fuzzy_collapsed}")
