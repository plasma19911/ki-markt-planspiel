...
// Beobachtet Tagesverlierer, verhindert fallende Messer und validiert Rebound-BUYs,
// erzwingt aber niemals einen Kauf am Profit-Optimizer vorbei.
...
function enrichInput(input,state){
  ...
  const policy='REBOUND-RADAR: Tagesverlierer sind ein eigener Beobachtungspool. Niemals allein wegen eines Kursverlusts kaufen. reboundWatch=true braucht bestaetigte Umkehr: kurzfristiges Momentum positiv, Abverkauf stabilisiert, kein STRONG-Sell/REVERSAL, kein HIGH-Event-Risiko und keine stark negativen News. Fallende Messer bleiben HOLD. Bestaetigte Rebounds konkurrieren normal im Profit-Optimizer mit Momentum-/Forward-Setups; der Rebound-Layer darf keinen BUY erzwingen. 15/30/60-Minuten-Lernen entscheidet separat fuer REBOUND_REVERSAL, welche Muster groesser werden duerfen oder geblockt werden. ';
...
function postProcess(r,input,state,storage){
  ...
  const cMap=new Map(candidates.map(c=>[key(c),c])),rm=reboundMap(state),out=[];let blocked=0,validated=0;
  for(const a of arr(plan.actions)){
    ...
    if(String(a?.action||'').toUpperCase()==='BUY'&&rw&&c){
      ...
      if(!learn.allowed){blocked++;out.push({symbol:sym,action:'HOLD',confidence:clamp(num(a.confidence,.55),.5,.8),allocation_pct:0,reason:`REBOUND-BLOCK: ${learn.block?learn.reason:'noch keine bestaetigte Umkehr'} · Qualität ${e.quality.toFixed(2)} · ${e.why.slice(0,4).join(' · ')}`});continue}
      const learnedCap=Math.max(10,35*num(learn.sizeMultiplier,1)),pct=Math.min(learnedCap,Math.max(10,num(a.allocation_pct,20)));
      validated++;out.push({...a,allocation_pct:+pct.toFixed(2),confidence:clamp(num(a.confidence,.6)+num(learn.confidenceDelta),.55,.9),reason:`${String(a.reason||'').slice(0,165)} · REBOUND bestätigt ${e.quality.toFixed(2)} · Lern-Score ${num(learn.scoreDelta).toFixed(2)} · Einstieg max. ${learnedCap.toFixed(0)}%`});continue
    }
    out.push(a);
  }
  plan.actions=out;
  plan.summary=`${String(plan.summary||'').slice(0,190)} · REBOUND-RADAR: ${rm.size} Verlierer beobachtet; ${validated} BUY validiert, ${blocked} fallende/ungeklärte Rebounds blockiert; kein Zwangskauf.`;
