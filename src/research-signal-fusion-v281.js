...
function defaults(){return{version:2,snapshots:{},newsSeen:{},positionScores:{},lastRanking:[],recent:[],stats:{researchBuys:0,hardBlocks:0,scoreWatches:0},updatedAt:null}}
...
function scoreStage(score){const n=num(score);return n>=72?'BUY_READY':n>=64?'CONFIRM':n>=58?'WATCH':'WEAK'}
...
function scoreRow(symbol,f,at,extra={}){return{symbol,fusionScore:f.score,stage:scoreStage(f.score),parts:f.parts,at,...extra}}
...
export function enforceResearchSignalFusionV281(plan,state={},storage=null,now=Date.now()){
 ...
 const mem={...defaults(),...read(storage,defaults())};mem.snapshots={...(mem.snapshots||{})};mem.newsSeen={...(mem.newsSeen||{})};mem.positionScores={...(mem.positionScores||{})};mem.lastRanking=arr(mem.lastRanking);mem.recent=arr(mem.recent);mem.stats={...defaults().stats,...(mem.stats||{})};
 const candidates=new Map(arr(state?.candidates).map(c=>[key(c),c])),positionMap=new Map(arr(state?.positions).map(p=>[key(p),p]).filter(([s])=>Boolean(s))),positions=new Set(positionMap.keys()),prior={...mem.snapshots},pf=portfolio(state),ranked=[];
 for(const [s,p] of positionMap){const old=prior[s];if(!mem.positionScores[s]&&Number.isFinite(Number(old?.fusionScore)))mem.positionScores[s]={symbol:s,fusionScore:num(old.fusionScore),entryScore:num(old.fusionScore),stage:scoreStage(old.fusionScore),parts:old.parts||{},at:num(old.at,now),source:'LAST_PRE_ENTRY'}}
 for(const [s,c] of candidates){
  if(!s)continue;
  const f=fusion(c,state,prior[s],mem,now),snapshot={at:now,fusionScore:f.score,score:f.m.score,confidence:f.m.confidence,m20:f.m.m20,accel:f.m.accel,price:f.m.price,parts:f.parts};mem.snapshots[s]=snapshot;
  if(positions.has(s)){const old=mem.positionScores[s]||{};mem.positionScores[s]=scoreRow(s,f,now,{entryScore:Number.isFinite(Number(old.entryScore))?Number(old.entryScore):f.score,source:'LIVE'});continue}
  ranked.push({s,c,f});
 }
 ranked.sort((a,b)=>b.f.score-a.f.score||b.f.m.score-a.f.m.score);const bySymbol=new Map(ranked.map(x=>[x.s,x]));
 const counters={researchBuys:0,hardBlocks:0,scoreWatches:0,existingBuys:0};const out=[];let newBuys=0;const hardBlocks=new Set(),nextPositions=new Set(positions);
 for(const raw of plan.actions){const s=key(raw),action=String(raw?.action||'').toUpperCase(),reason=String(raw?.reason||''),row=bySymbol.get(s);
  if(action==='BUY'){counters.existingBuys++;nextPositions.add(s);if(row)mem.positionScores[s]=scoreRow(s,row.f,now,{entryScore:row.f.score,source:'ENTRY'});out.push(raw);continue}
  if(action==='SELL'){nextPositions.delete(s);out.push(raw);continue}
  if(action!=='HOLD'||!row||positions.has(s)){out.push(raw);continue}
  const {c,f}=row;if(hardBlocked(c,f.m,reason)){counters.hardBlocks++;hardBlocks.add(s);out.push(raw);continue}
 ...
 mem.lastRanking=ranked.slice(0,24).map(x=>({...scoreRow(x.s,x.f,now),hardBlocked:hardBlocks.has(x.s)}));
 for(const [s,v] of Object.entries(mem.positionScores))if(!nextPositions.has(s)&&now-num(v?.at,0)>45*60000)delete mem.positionScores[s];
 for(const [s,v] of Object.entries(mem.snapshots))if(now-num(v?.at,0)>30*60000&&!candidates.has(s)&&!nextPositions.has(s))delete mem.snapshots[s];for(const [h,t] of Object.entries(mem.newsSeen))if(now-num(t,0)>48*3600*1000)delete mem.newsSeen[h];if(mem.recent.length>160)mem.recent=mem.recent.slice(-160);mem.version=2;mem.updatedAt=new Date(now).toISOString();write(storage,mem);
 ...
 const positionScores=[...nextPositions].map(s=>mem.positionScores[s]).filter(Boolean).map(x=>({...x,stage:scoreStage(x.fusionScore)}));
 return{plan,counters,state:mem,ranking:mem.lastRanking,positionScores};
}
