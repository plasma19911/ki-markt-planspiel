...
export function severeWeaknessV308(p={}){
  const c=FINAL_SELL_AUTHORITY_V308,stable=num(p?.decisionScore,p?.score,50),raw=num(p?.rawDecisionScore,stable),entry=num(p?.entryDecisionScore,stable),stableRawGap=stable-raw,entryDeterioration=entry-stable,direction=String(p?.chartDirectionMode||'').toUpperCase(),chart=num(p?.chartMoveFromEntryPct,0);
  const strongUptrend=direction==='UP'&&chart>=c.strongUptrendProtectionPct&&raw>c.directSellRawScore;
  const severe=raw<=c.directSellRawScore||(raw<=c.severeRawScore&&(stableRawGap>=c.minStableRawDivergence||entryDeterioration>=c.minEntryDeterioration));
  return{severe:severe&&!strongUptrend,stable,raw,entry,stableRawGap,entryDeterioration,direction,chart,strongUptrend};
}

export function enforceFinalSellAuthorityV308(plan,state={}){
  if(!plan||!Array.isArray(plan.actions))return{plan,counters:{sellLocks:0,forcedWeakSells:0}};
  const actions=plan.actions.map(a=>({...a})),idx=new Map();actions.forEach((a,i)=>{const s=key(a);if(s&&!idx.has(s))idx.set(s,i)});
  const counters={sellLocks:0,forcedWeakSells:0};
  for(const p of arr(state?.positions)){
    ...
    const w=severeWeaknessV308(p);if(!w.severe)continue;
    const forced={...(a||{}),symbol:s,name:p?.name,action:'SELL',allocation_pct:0,confidence:.96,finalSellAuthorityV308:true,forcedWeakSellV308:true,reason:`V30.8 SOFORT-SELL: ${s} ist deutlich deterioriert. RawScore ${w.raw.toFixed(1)}/100, geglätteter Score ${w.stable.toFixed(1)}/100 (Abstand ${w.stableRawGap.toFixed(1)}), Einstiegsscore ${w.entry.toFixed(1)} (Verschlechterung ${w.entryDeterioration.toFixed(1)}), Chart ${w.chart>=0?'+':''}${w.chart.toFixed(2)}%, Richtung ${w.direction||'UNBEKANNT'}. Hysterese darf diesen Exit nicht mehr verdecken.`};
    if(i===undefined){i=actions.length;idx.set(s,i);actions.push(forced)}else actions[i]=forced;
    counters.forcedWeakSells++;
  }
  plan.actions=actions;plan.summary=`${String(plan.summary||'').slice(0,140)} · V30.8 Final-Sell: ${counters.sellLocks} SELL fixiert · ${counters.forcedWeakSells} Schwäche-Sofortverkauf.`;
...
export class FinalSellAuthorityV308{
  ...
  async run(model,input){const legacy=input===undefined&&model&&typeof model==='object',payload=legacy?model:input,r=legacy?await this.inner.run(payload):await this.inner.run(model,payload);if(!isTradingInput(payload))return r;const p=parsePlan(r);if(!p)return r;const state=typeof this.getState==='function'?(this.getState()||{}):{},out=enforceFinalSellAuthorityV308(p,state);this.latest=out;return encode(r,out.plan)}
  status(){return{enabled:true,...FINAL_SELL_AUTHORITY_V308,mode:'outermost-final-sell-authority',sellCannotBeDowngradedToHold:true,finalSellSameScan:true,severeWeaknessOverridesScoreHysteresis:true,latest:this.latest?.counters||null,rule:'Ein finales SELL bleibt SELL. Stark deteriorierte Bestandspositionen mit sehr niedrigem RawScore werden im selben Entscheidungszyklus verkauft; eine freundliche Hysterese darf den Exit nicht mehr verdecken.'}}
}
