...
export const FINAL_SELL_AUTHORITY_V308={
  version:30.8,
  patch:'30.8.1-fresh-position-sell-guard',
  severeRawScore:35,
...
export function severeWeaknessV308(p={}){
  ...
  const confirmed=extremeRaw||chartBreak;
...
export function enforceFinalSellAuthorityV308(plan,state={},now=Date.now()){
  ...
  for(const p of arr(state?.positions)){
    ...
    // Ein SELL, das alle inneren Reife-/Risiko-/Candle-Guards bereits ueberstanden hat,
    // bleibt final. V30.8 darf dieses bestaetigte SELL nicht wieder zu HOLD machen.
    if(act==='SELL'){
    ...
    if(w.rawSevere&&!w.confirmed){
      ...
      if(i!==undefined&&act==='HOLD')actions[i]={...a,reason:`${String(a?.reason||'HOLD').slice(0,520)} · V30.8.1: niedriger RawScore allein ist kein Sofortverkauf; Verkäufer-/Chartbruch ist noch nicht bestätigt.`};
      continue;
    }
    if(!w.severe)continue;
    const age=ageMinutes(p,now);
    // Kritischer V30.8.1-Fix: Die aeussere RawScore-Schicht darf den bereits vorhandenen
    // Trade-Maturity-Schutz nicht mehr ueberfahren. Echte harte/strukturelle SELLs kommen
    // aus den inneren Guards bereits als SELL oben an und wurden im Block darueber fixiert.
    // Ein von V30.8 selbst neu erzeugtes Score-SELL wartet bei frischen Positionen.
    if(age<c.freshProtectionMinutes){
      counters.freshWeakSellsBlocked++;
      if(i!==undefined)actions[i]={...(a||{}),symbol:s,name:p?.name,action:'HOLD',allocation_pct:0,confidence:Math.max(.76,num(a?.confidence,.76)),finalSellAuthorityV308:true,freshPositionSellGuardV3081:true,reason:`V30.8.1 FRESH-POSITION-SCHUTZ: ${s} ist erst ${age.toFixed(1)} Min. offen. Ein neu erzeugtes RawScore-SELL darf die Reife-/Candle-Guards nicht mehr umgehen. Echter harter Risiko- oder Struktur-SELL bleibt sofort ausführbar.`};
      continue;
    }
...
export class FinalSellAuthorityV308{
  ...
  status(){return{enabled:true,...FINAL_SELL_AUTHORITY_V308,mode:'outermost-final-sell-authority',sellCannotBeDowngradedToHold:true,finalSellSameScan:true,severeWeaknessOverridesScoreHysteresis:true,freshPositionRawScoreProtection:true,forcedWeaknessNeedsChartConfirmation:true,latest:this.latest?.counters||null,rule:'Ein von den inneren Reife-/Risiko-/Candle-Guards bestätigtes SELL bleibt SELL. V30.8.1 erzeugt aber keinen neuen Sofortverkauf mehr nur wegen eines kurzfristig niedrigen RawScores: frische Positionen sind 20 Minuten gegen Score-only-Churn geschützt und danach braucht die RawScore-Schwäche zusätzlich Chart-/Verkäuferbestätigung (außer bei extremem RawScore).'}}
}
