...
export const ENTRY_PROFIT_V290={
  ...
  profit:{
    ...
    largePeakPct:3,
    tiers:[
      {peakMin:6,givebackFraction:.25,minGivebackPoints:.85,maxHoldScore:64},
      {peakMin:3,givebackFraction:.32,minGivebackPoints:.70,maxHoldScore:60},
      {peakMin:1.5,givebackFraction:.40,minGivebackPoints:.55,maxHoldScore:56},
      {peakMin:.8,givebackFraction:.50,minGivebackPoints:.45,maxHoldScore:50}
    ]
  }
};
...
export function trendV290(history=[],score=0,now=Date.now()){
  ...
  return{delta1:+delta1.toFixed(1),deltaWindow:+deltaWindow.toFixed(1),rising,strongRising,falling,strongFalling,confirmed68,confirmed72,samples:rows.length+1};
}
...
export function entryAllocationPctV290(cash=0,decision={}){
  ...
  let lo=4,hi=6,pct=4;
  if(tier==='EARLY'){lo=5;hi=8;pct=5+Math.max(0,score-68)*.35}
  ...
  else pct=4+Math.max(0,score-65)*.30;
...
export function profitDecisionV290({pnlPct=0,peakPnlPct=0,holdScore=50,peakHoldScore=50,lastHoldScore=50,coverage=0,partial=false,ageMinutes=999,m5=0,m20=0,acc=0,momentumState='',momentumSellSignal=''}={}){
  ...
  const scoreWeak=score<=tier.maxHoldScore&&(scoreDelta<=-3||scoreFromPeak<=-7);
  const nearPeakExhaustion=peak>=2.5&&givebackFraction>=.14&&scoreFromPeak<=-9&&momentumWeak&&score<=62;
  if(nearPeakExhaustion)return{...base,action:'SELL',reason:'near_peak_exhaustion',label:'Gewinn sichern · Anstieg läuft aus'};
  if(trailHit&&scoreWeak&&momentumWeak)return{...base,action:'SELL',reason:'dynamic_profit_lock',label:'Gewinn sichern · dynamischer Rücklauf'};
