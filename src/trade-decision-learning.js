...
function newsRowsFor(symbol,state){const b=sym(symbol).split('.')[0],rows=[];for(const n of arr(state?.newsRadar))if(sym(n?.symbol).split('.')[0]===b)rows.push(n);for(const e of arr(state?.newsLearning?.events))if(sym(e?.symbol).split('.')[0]===b)rows.push({headline:e.headline});return rows}
function recompute(l){
 ...
 l.summary={evaluatedTrades:tradeN,buyTooEarly:buyEarly,sellTooEarly:sellEarly,goodExits,missedNewsShock:missed,entryPatienceMultiplier:+clamp(1+entryPressure*.45,.82,1.28).toFixed(3),exitPatienceMultiplier:+clamp(1+exitPressure*.45,.82,1.30).toFixed(3),newsShockRetestMultiplier:+clamp(1+newsPressure,.78,1.22).toFixed(3),note:'Adaptive Faktoren entstehen aus realem Nachlauf nach Kauf/Verkauf. Sie ändern Geduld graduell; keine starre Gewinn-/Verlustschwelle.'};
...
export async function updateTradeDecisionLearning(storage,state={},limit=6){
 ...
 // Verpasste strukturell positive News-Schocks separat erfassen.
 const bought=new Set(pairs.map(x=>x.symbol));for(const c of arr(state?.candidates).slice(0,25)){const s=sym(c?.symbol);if(!s||bought.has(s))continue;const impact=strongestNewsImpact(newsRowsFor(s,state)),day=num(c?.day,c?.day_change);if(!(impact.direction>0&&impact.impact>=4&&Math.abs(day)>=8))continue;const id=`${date}:${s}:MISSED_NEWS_SHOCK`;if(l.seen[id])continue;l.seen[id]=1;l.samples.push({id,kind:'MISSED',date,symbol:s,missedNewsShock:true,eventType:impact.type,dayMovePct:+day.toFixed(2),headline:String(impact.headline||'').slice(0,180)});}
