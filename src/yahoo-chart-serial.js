...
const INSTALL_KEY='__kiYahooChartSerialV2';
if(!globalThis[INSTALL_KEY]){
 ...
 async function pump(){if(active||!queue.length)return;active=true;while(queue.length){const x=queue.shift();try{const wait=Math.max(0,450-(Date.now()-lastStart));if(wait)await sleep(wait);lastStart=Date.now();x.resolve(await nativeFetch(x.input,x.init))}catch(e){x.reject(e)}}active=false}
