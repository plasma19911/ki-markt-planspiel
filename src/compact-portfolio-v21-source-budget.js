...
export class MarketPortfolio extends BasePortfolio{
 ...
 async status(){
  ...
  s.sourceBudgetPolicy={enabled:true,version:21.3,pcWideCoveragePreserved:true,earlyDipLiveWave:LIVE_EARLY_WAVE,secondChanceLiveWave:3,slowIntelligenceCached:true,oneMinuteCacheSeconds:45,yahooChartPacingMs:450,catalystGapRequestReserve:1,rule:'PC scannt weiterhin das Volluniversum. Cloudflare verteilt teure 1m-Zusatzchecks rotierend ueber Minuten; die Early-Dip-Live-Welle ist auf zwei Werte begrenzt. Dadurch bleibt mehr Requestbudget fuer die finalen Kerzen-, Mehr-Zeitebenen- und gezielten News-Pruefungen, ohne Kandidaten aus dem Breitscan zu verlieren.'};
  ...
  if(s.profitOptimizer)s.profitOptimizer={...s.profitOptimizer,todayTradeReviewV2:true,deeperDipPreferred:true,realDipThresholdPct:.70,redDayShallowRetestBlocked:true,missingMtfShallowBuyBlocked:true,noiseSellBlocked:true,reentryNeedsNewStructure:true,continuationCannotBypassMtf:true,catalystGapDiscoveryV2:true,sourceBudgetVersion:21.3};
  if(s.executionModel)s.executionModel={...s.executionModel,todayTradeReviewV2:true,microDipBuyBlocked:true,microDipThresholdPct:.70,redDayShallowRetestBlocked:true,missingMtfShallowBuyBlocked:true,highChaseBlocked:true,noiseSellBlocked:true,catalystGapDiscoveryV2:true,sourceBudgetVersion:21.3};
  return s;
 }
}
