// Short-lived in-isolate intelligence snapshot cache. Fast price decisions stay fresh;
// slowly changing context (RSS, event calendar, daily/weekly charts) is reused across
// minute scans. Only fully materialized response bytes are shared across requests.
// Request-owned fetch Promises are deliberately never stored globally.
const INSTALL_KEY='__kiIntelligenceRequestCacheV3';
const STATS_KEY='__kiIntelligenceRequestCacheStatsV3';
const MAX_ITEMS=260;
const cache=new Map();
...
function cacheKey(u){const x=new URL(u);x.hostname='intelligence-cache.local';return x.pathname+'?'+[...x.searchParams.entries()].sort((a,b)=>a[0].localeCompare(b[0])||String(a[1]).localeCompare(String(b[1]))).map(([k,v])=>`${encodeURIComponent(k)}=${encodeURIComponent(v)}`).join('&')}
...
if(!globalThis[INSTALL_KEY]){
 ...
 const stats=globalThis[STATS_KEY]={hits:0,misses:0,stored:0,staleRemoved:0,lastHitAt:null,lastStoreAt:null,oneMinuteTtlSeconds:45,crossRequestPromiseSharing:false};
 globalThis.fetch=async function intelligenceRequestCacheFetch(input,init){
  ...
  const k=cacheKey(u),now=Date.now(),old=cache.get(k);
  if(old&&now<old.expiresAt){stats.hits++;stats.lastHitAt=new Date().toISOString();return restore(old.snap)}
  if(old){cache.delete(k);stats.staleRemoved++}
  stats.misses++;
  const r=await nativeFetch(input,init),snap=await snapshot(r);
  if(r.ok){cache.set(k,{expiresAt:Date.now()+ttl,snap});stats.stored++;stats.lastStoreAt=new Date().toISOString();prune()}
  return restore(snap);
 };
}
