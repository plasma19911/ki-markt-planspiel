...
const INSTALL_KEY='__kiIntelligenceRequestCacheV1';
const STATS_KEY='__kiIntelligenceRequestCacheStatsV1';
const MAX_ITEMS=240;
...
function cacheKey(u){const x=new URL(u);return x.origin+x.pathname+'?'+[...x.searchParams.entries()].sort((a,b)=>a[0].localeCompare(b[0])||String(a[1]).localeCompare(String(b[1]))).map(([k,v])=>`${encodeURIComponent(k)}=${encodeURIComponent(v)}`).join('&')}
...
if(!globalThis[INSTALL_KEY]){
 ...
 const stats=globalThis[STATS_KEY]={hits:0,inflightHits:0,misses:0,stored:0,staleRemoved:0,lastHitAt:null,lastStoreAt:null};
