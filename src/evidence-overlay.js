...
export function applyEvidenceDiversity(fast,prompt){
  ...
  for(const c of fast.context||[]){
    ...
    const activeReady=idlePortfolio&&c.fastAction==='HOLD'&&hs.buy>=activeBuyFloor&&hs.sell<1.55&&tech.fresh===true&&num(tech.vwapDistancePct)>-.08&&num(tech.adx)>=12&&num(tech.plusDI)>=num(tech.minusDI)*.85&&num(mtf.longVotes)>=1&&num(mtf.shortVotes)<3&&(c.regime!=='TREND_DOWN'||num(mtf.longVotes)>=2)&&hardSafe&&num(raw.news)>-.50&&num(raw.liveScore)>=0&&num(raw.liveConfidence,.45)>=.40;
    if(activeReady){const confidence=Math.max(.55,Math.min(.69,.50+hs.buy*.035+Math.max(0,num(raw.liveConfidence)-.4)*.2+Math.max(0,num(c.marketRelative20m))*.04));activeFirst.push({symbol:c.symbol,action:'BUY',confidence,allocation_pct:+activeFirstAllocation(style,cash).toFixed(1),reason:`ACTIVE-FIRST-ENTRY: bestes sicheres Setup im leeren Depot · BUY-Score ${hs.buy.toFixed(1)} · ADX ${num(tech.adx).toFixed(0)} · MTF ${num(mtf.longVotes)}/4 positiv · VWAP-Abstand ${num(tech.vwapDistancePct).toFixed(2)}% · harte Sicherheitschecks bestanden`,rank:hs.buy+num(raw.liveScore)*.20+num(raw.liveConfidence)*.45+num(mtf.longVotes)*.18+num(tech.adx)*.012+num(c.marketRelative20m)*.12})}

    // Letzte Stufe für ein leeres Paper-Depot: Wenn kein stärkeres Setup existiert,
    // darf der beste vollständig geprüfte, nicht-bärische Kandidat eine kleine Startposition
    // eröffnen. Echte Hard-Blocks bleiben unangetastet. So wird "bester verfügbarer Wert"
    // tatsächlich gehandelt, ohne den alten unsicheren 100%-Fallback wieder einzuschalten.
    const deployReady=idlePortfolio&&c.fastAction==='HOLD'&&hs.sell<1.70&&tech.fresh===true&&num(tech.vwapDistancePct)>-.25&&num(tech.adx)>=8&&num(mtf.shortVotes)<3&&hardSafe&&num(raw.news)>-.60&&num(raw.liveScore)>=0&&num(raw.liveConfidence,.45)>=.40;
    if(deployReady){const confidence=Math.max(.52,Math.min(.64,.48+Math.max(0,hs.buy)*.025+num(raw.liveConfidence)*.12+Math.max(0,num(mtf.longVotes))* .015));cashDeploy.push({symbol:c.symbol,action:'BUY',confidence,allocation_pct:+cashDeployAllocation(style,cash).toFixed(1),reason:`BEST-SAFE-CASH-DEPLOY: bestes vollständig geprüftes, nicht-bärisches Setup · BUY ${hs.buy.toFixed(1)} / SELL ${hs.sell.toFixed(1)} · ADX ${num(tech.adx).toFixed(0)} · VWAP ${num(tech.vwapDistancePct).toFixed(2)}% · keine harte Risikosperre`,rank:hs.buy-hs.sell*.8+num(raw.liveScore)*.30+num(raw.liveConfidence)*.50+num(mtf.longVotes)*.10-num(mtf.shortVotes)*.12+num(c.marketRelative20m)*.10})}
  }
