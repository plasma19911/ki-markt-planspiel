...
export function applyEvidenceDiversity(fast,prompt){
  if(!fast)return fast;
  const candidates=parseJsonBetween(prompt,'Kandidaten=',' Gehalten='),held=parseJsonBetween(prompt,' Gehalten='),cm=new Map((Array.isArray(candidates)?candidates:[]).map(x=>[String(x.symbol||'').toUpperCase(),x])),gap=new Map((fast.gapContext||[]).map(x=>[String(x.symbol||'').toUpperCase(),x])),ratios=fast?.volumeConfirmation?.ratios||{},minVol=num(fast?.volumeConfirmation?.minRatio,FAST_CALIBRATION.minRelativeVolume||1.10),context=[],evidenceMap=new Map(),continuation=[],opportunities=[],activeFirst=[];
  ...
  for(const c of fast.context||[]){
    ...
    // Aktiver Ersteinstieg nur für ein komplett leeres Paper-Depot. Das ist bewusst
    // lockerer als ein normaler FAST-BUY, aber weiterhin kein blindes Fallback: aktuelle
    // technische Daten, mindestens eine positive Zeitebene, kein starkes Gegensignal,
    // keine Ausführungs-/Liquiditäts-/FX-/Gap-Sperre und ein nicht-negatives Live-Setup.
