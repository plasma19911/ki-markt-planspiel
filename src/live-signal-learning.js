...
function contextMap(fast,prompt){
  ...
  for(const c of arr(fast?.context)){
    ...
    out.set(k,{signature:`${c.regime||'UNKNOWN'}|${bucket}|${gap}`,regime:c.regime||'UNKNOWN',mtf:a,gap,timingBucket:classifyEntryTiming(candidate,c),timingSnapshot:{day:num(candidate?.day),m5:num(candidate?.intraday5m),m20:num(candidate?.intraday20m),rsi:num(candidate?.intradayRsi,50),drawdown20m:num(candidate?.drawdownFrom20mHighPct),breakoutScore:num(candidate?.momentumBreakoutScore),volumeRatio:num(candidate?.volumeRatio,1),vwapDistancePct:num(c?.technical?.vwapDistancePct),priceVsResistancePct:num(c?.technical?.priceVsResistancePct)}})
  }
  return out;
}
function defaults(){return{version:2,open:{},pending:{},stats:{},completed:0,timingStats:{},timedCompleted:0,recentTiming:[]}}
...
function adjustmentFromState(state,bucket){
  ...
  }else if(a.quality>.18&&a.winRate!=null&&a.winRate>.55){out.confidenceDelta=.025;out.scoreDelta=clamp(.25+a.quality*.35,.25,.65);out.sizeMultiplier=1.08;out.reason=`Timing-Lernen bestätigt ${bucket}: ${a.samples15} Fälle, Ø ${a.quality.toFixed(2)}%, Treffer ${(a.winRate*100).toFixed(0)}%`}
  return out
}
...
export function getLiveLearningStatus(storage){
  ...
  return{version:2,mode:'15/30/60-minute-entry-timing-learning',activePositions:Object.keys(state.open||{}).length,pendingEntries:Object.keys(state.pending||{}).length,completedTradeOutcomes:num(state.completed),timedCheckpoints:num(state.timedCompleted),horizonsMinutes:HORIZONS,minSamplesBeforeAdjustment:MIN_TIMING_SAMPLES,hardBlockSamples:HARD_TIMING_SAMPLES,matureTimingBuckets:buckets.filter(x=>x.samples15>=MIN_TIMING_SAMPLES).length,buckets:buckets.slice(0,12),recentTiming:arr(state.recentTiming).slice(-12).reverse()}
}
...
export function applyLiveOutcomeLearning(fast,prompt,storage){
  ...
  state.version=2;state.updatedAt=now;write(storage,state);const learned=Object.entries(state.stats).filter(([,v])=>num(v.count)>=12).length,timingMature=Object.keys(state.timingStats).filter(x=>timingAggregate(state,x).samples15>=MIN_TIMING_SAMPLES).length;
  return{...fast,actions,liveLearning:{completedOutcomes:num(state.completed),matureSetupBuckets:learned,minOutcomesPerBucket:12,timedCheckpoints:num(state.timedCompleted),matureTimingBuckets:timingMature,timingHorizonsMinutes:HORIZONS,minTimingSamples:MIN_TIMING_SAMPLES,mode:'trade-outcome + 15/30/60-minute entry-timing learning'}}
}
