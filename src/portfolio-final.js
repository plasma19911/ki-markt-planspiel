...
export class MarketPortfolio extends ProdPortfolio{
  ...
  async start(o={}){
    return super.start({...o,includeEtfs:true,includeLeverage:false});
  }
  ...
  async aiPlan(cands,ps,cfg){
    ...
    const allowed=(cands||[]).filter(x=>x.type!=='LEVERAGED_ETF');
    const heldAllowed=(ps||[]).filter(x=>x.instrument_type!=='LEVERAGED_ETF');
    if(!allowed.length)return{summary:'NEWS-ONLY bzw. keine frischen handelbaren Kurse – keine Orderentscheidung.',actions:[]};
    const model=this.executionModel(cfg),data=allowed.slice(0,12).map(x=>({symbol:x.symbol,type:x.type,theme:x.theme,score:+num(x.score).toFixed(2),confidence:+num(x.confidence).toFixed(2),day:+num(x.dayChange).toFixed(2),m5:+num(x.momentum5).toFixed(2),m20:+num(x.momentum20).toFixed(2),rsi:x.rsi==null?null:+num(x.rsi).toFixed(1),news:+num(x.newsScore).toFixed(2),newsConfidence:+num(x.newsConfidence).toFixed(2),eventRisk:x.eventRisk,pro:(x.pro||[]).slice(0,4),contra:(x.contra||[]).slice(0,4),headlines:(x.headlines||[]).slice(0,2)}));
    const held=heldAllowed.map(p=>({symbol:p.symbol,type:p.instrument_type,theme:p.theme,invested:num(p.invested),pnlPct:num(p.invested)?+((positionValue(p)/num(p.invested)-1)*100).toFixed(2):0,score:p.score}));
    const prompt=`PAPER-TRADING ONLY. Keine echten Orders. Aktuell verfügbares Spielgeld ${num(cfg.cash).toFixed(2)} ${cfg.currency}. Erlaubt sind ausschliesslich Aktien und normale ETFs; Hebel-/Inverse-Produkte sind verboten. Es gibt KEINE harte Grenze für Anzahl gleichzeitiger Positionen, Haltedauer, Branche, Reserve, Mindestorder oder Verlustserie. Die einzige harte Portfoliogrenze ist das aktuell vorhandene Cash inklusive Gebühren und Slippage. allocation_pct ist der Prozentanteil des AKTUELL VERFÜGBAREN CASH. Die Summe neuer BUY-Allokationen darf 100% des verfügbaren Cash nicht überschreiten, aber du darfst frei konzentrieren oder auf beliebig viele Kandidaten verteilen. Kosten ${model.feeFixed.toFixed(2)} ${cfg.currency} je Kauf/Verkauf, normaler Ausführungspuffer ${model.slippagePercent.toFixed(2)}%. Berücksichtige frische Kurse, News, Events, FX und Kosten. Gib keine versteckten Gedankengänge aus, nur kurze Begründungen. JSON {"summary":"kurz","actions":[{"symbol":"TICKER","action":"BUY|SELL|HOLD","confidence":0.0,"allocation_pct":0,"reason":"1 Satz"}]}. BUY nur Kandidaten, SELL nur gehaltene Werte. Kandidaten=${JSON.stringify(data)} Gehalten=${JSON.stringify(held)}`;
  ...
  open(cand,pct,reason){
    if(cand?.type==='LEVERAGED_ETF')return false;
  ...
  async status(){const s=await super.status();s.sourceHealth=(s.sourceHealth||[]).filter(x=>!REMOVED_SOURCES.has(x.source));s.candidates=(s.candidates||[]).filter(x=>x.instrument_type!=='LEVERAGED_ETF');s.newsRadar=(s.newsRadar||[]).filter(x=>x.instrument_type!=='LEVERAGED_ETF');if(s?.config){s.config.include_etfs=1;s.config.include_leverage=0}return s}

  async lastWeek(){const c=this.cfg(),m=this.executionModel(c);return runLastWeekHindsight(this.env,{includeEtfs:true,includeLeverage:false,feeFixed:m.feeFixed,feePercent:m.feePercent,slippagePercent:m.slippagePercent,leveragedSlippagePercent:m.slippagePercent})}
}
