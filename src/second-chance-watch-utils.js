...
export const SECOND_CHANCE_TARGET=16;
...
export function isSecondChanceCandidate(c={}){
 const score=num(c?.score),conf=num(c?.confidence),good=(score>=4.55&&conf>=.62)||(score>=4.15&&conf>=.70)||(score>=3.75&&conf>=.77),bad=eventRisk(c)==='HIGH'||sellSignal(c)==='STRONG'||['REVERSAL','EXHAUSTION'].includes(stateName(c))||blockedSymbol(c?.symbol);
...
export function buildSecondChanceWatch(previous,current,now=Date.now()){
 ...
 return{version:4,updatedAt:stamp,candidateCount:candidates.length,target:SECOND_CHANCE_TARGET,retentionMinutes:12,recheckPerScan:SECOND_CHANCE_RECHECK_PER_SCAN,forcedBuy:false,requiresFreshOneMinuteRecheck:true,candidates};
}
