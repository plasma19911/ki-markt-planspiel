...
function qualityRank(a){
  const live=num(a?.liveScore),buy=num(a?.buyScore,a?.fastScore),sell=num(a?.sellScore),confidence=num(a?.confidence);
...
function bestValidatedFastBuy(fast,existingActions=[]){
  ...
  const blockedSymbols=new Set((existingActions||[]).filter(x=>String(x?.action||'').toUpperCase()==='SELL').map(x=>String(x?.symbol||'').toUpperCase()));
  return (fast?.actions||[])
...
export function enforceFastExecutionGuards(aiResponse,fast){
  ...
  try{
    const j=JSON.parse(raw.slice(a,b+1)),actions=Array.isArray(j.actions)?j.actions:[],ctx=new Map((fast.context||[]).map(x=>[String(x.symbol||'').toUpperCase(),x])),gaps=new Map((fast.gapContext||[]).map(x=>[String(x.symbol||'').toUpperCase(),x])),ratios=fast?.volumeConfirmation?.ratios||{},prevalidated=new Set((fast.actions||[]).filter(x=>String(x?.action||'').toUpperCase()==='BUY').map(x=>String(x?.symbol||'').toUpperCase())),minVolume=num(fast?.volumeConfirmation?.minRatio,FAST_CALIBRATION.minRelativeVolume||1.10),maxSpread=num(FAST_CALIBRATION.maxSpreadPct,.8),minAdx=num(FAST_CALIBRATION.minAdxBuy,18);
    ...
    const notes=[];if(blocked)notes.push(`${blocked} BUY durch Ausführungs-Schutz blockiert`);if(autoFastBuy)notes.push(`bester vollständig validierter BUY ${String(autoFastBuy.symbol||'')} (Qualitätsrang ${qualityRank(autoFastBuy).toFixed(2)}) übernommen`);if(notes.length)j.summary=`${String(j.summary||'KI-Plan').slice(0,300)} · ${notes.join(' · ')}.`;
