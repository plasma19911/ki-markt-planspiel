...
export function enforceFastExecutionGuards(aiResponse,fast){
  ...
  try{
    ...
    if(!hasExecutableBuy){
      ...
      if(autoFastBuy){
        j.actions.push({...autoFastBuy,allocation_pct:Math.max(0,Math.min(100,num(autoFastBuy.allocation_pct,0))),reason:`BEST-VALIDATED-BUY: ${String(autoFastBuy.reason||'Vollständig geprüfter Fast-BUY').slice(0,260)}`});
