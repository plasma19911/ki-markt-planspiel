...
export function enforceFastExecutionGuards(aiResponse,fast){
  ...
  try{
    ...
    const maxSpread=num(FAST_CALIBRATION.maxSpreadPct,.8),minAdx=num(FAST_CALIBRATION.minAdxBuy,18);
    j.actions=actions.map(action=>{
      if(String(action?.action||'').toUpperCase()!=='BUY')return action;
      const symbol=String(action?.symbol||'').toUpperCase(),c=ctx.get(symbol),g=gaps.get(symbol),blocks=[];let next={...action};
      ...
      if(c?.technical){
        if(c.technical.fresh!==true)blocks.push('technische Daten nicht frisch');
        if(num(c.technical.vwapDistancePct)<=0)blocks.push('Kurs nicht über VWAP');
        if(num(c.technical.adx)<minAdx)blocks.push(`ADX ${num(c.technical.adx).toFixed(1)} < ${minAdx.toFixed(1)}`);
      }else if(c)blocks.push('technischer Deep-Check fehlt');
      if(c&&num(c?.multiTimeframe?.longVotes)<2)blocks.push('weniger als 2 positive Zeitebenen');
      if(c?.regime==='TREND_DOWN'&&num(c?.multiTimeframe?.longVotes)<3)blocks.push('Abwärtsregime ohne 3/4 MTF-Gegenbestätigung');
      ...
      if(!hasVolume){next.confidence=Math.min(num(next.confidence,.5),.62);next.allocation_pct=+(num(next.allocation_pct)*.70).toFixed(1);next.reason=`Volumenbestätigung nicht verfügbar: Einsatz reduziert. ${String(next.reason||'').slice(0,220)}`}
      const cost=estimateAiBuyCost(fast,next.allocation_pct);
      if(cost&&(!Number.isFinite(cost.costPct)||cost.costPct>cost.maxRoundTripCostPct))blocks.push(`geschätzte Roundtrip-Kosten ${Number.isFinite(cost.costPct)?cost.costPct.toFixed(1):'n/a'}% > ${cost.maxRoundTripCostPct.toFixed(1)}%`);
