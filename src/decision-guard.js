...
export function enforceFastExecutionGuards(aiResponse,fast){
  ...
  try{
    ...
    j.actions=actions.map(action=>{
      ...
      const cost=estimateAiBuyCost(fast,next.allocation_pct);if(cost&&(!Number.isFinite(cost.costPct)||cost.costPct>cost.maxRoundTripCostPct))blocks.push(`geschätzte Roundtrip-Kosten ${Number.isFinite(cost.costPct)?cost.costPct.toFixed(1):'n/a'}% > ${cost.maxRoundTripCostPct.toFixed(1)}%`);
