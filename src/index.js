...
import {MarketPortfolio} from './portfolio-learning.js';
...
export default{
 async fetch(request,env){
  ...
  try{
   ...
   if(u.pathname==='/api/start'&&request.method==='POST'){const b=await request.json().catch(()=>({}));const r=await p.start({...b,includeEtfs:true,includeLeverage:false});await p.scan();return reply(r)}
