...
// V30.7 adds dashboard manual BUY/SELL nudges for the paper portfolio.
...
async function manualTradeResponse(request,env){
 ...
 const p=env.PORTFOLIO.getByName('default-paper-portfolio'),body=await request.json().catch(()=>({})),intent=await p.manualTradeIntent(body);if(!intent?.ok)return Response.json(intent,{status:intent?.status||400,headers:{'cache-control':'no-store'}});let scan=null;try{scan=await p.scan()}catch(e){return Response.json({...intent,ok:false,error:`Manueller Impuls gespeichert, Sofort-Scan fehlgeschlagen: ${String(e?.message||e)}`},{status:500,headers:{'cache-control':'no-store'}})}return Response.json({...intent,scanTriggered:true,scan},{headers:{'cache-control':'no-store'}})
}
export default{
