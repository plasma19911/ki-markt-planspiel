...
export class MarketPortfolio extends BasePortfolio{
  ...
  _withNewsCatalystStatus(s={}){
    ...
    const newsLearning=s.newsLearning?.summary||{};
    s.newsCatalystPolicy={...guard,updatedAt:this.__newsCatalystV31710?.updatedAt||null,targets:this.__newsCatalystV31710?.targets||0,lookups:this.__newsCatalystV31710?.lookups||0,urgentTargets:this.__newsCatalystV31710?.urgentTargets||0,pipeline:this.__newsCatalystV31710?.pipeline||{detected:0,companyMatched:0,fresh:0,marketConfirmed:0,waiting:0,blocked:0,used:0},reactionLearning:{version:31.729,regionalBenchmarks:arr(newsLearning.regionalBenchmarks),evaluatedEvents:Number(newsLearning.evaluatedEvents)||0,pendingEvents:Number(newsLearning.pendingEvents)||0,baselineEvents:Number(newsLearning.baselineEvents)||0,evaluationBatchSize:Number(newsLearning.lastEvaluationBatchSize)||0,evaluationQuoteCount:Number(newsLearning.lastEvaluationQuoteCount)||0,evaluationAttempts:Number(newsLearning.lastEvaluationAttemptCount)||0,provider:newsLearning.lastEvaluationProvider||null,lastError:newsLearning.lastEvaluationError||null,httpStatuses:arr(newsLearning.lastEvaluationHttpStatuses),windowsMinutes:[15,60,240,360],directionalSamples:Number(newsLearning.reactionLag?.directionalSamples)||0,averageDirectionalMinutes:Number.isFinite(Number(newsLearning.reactionLag?.avgDirectionalMinutes))?Number(newsLearning.reactionLag.avgDirectionalMinutes):null,statisticalNotPredictiveCertainty:true},refreshError:this.__newsCatalystV31710?.refreshError||null,symbols:arr(this.__newsCatalystV31710?.symbols).map(x=>({symbol:x.symbol,headline:x.headline,eventType:x.eventType,direction:x.direction,impact:x.impact,importance:x.importance,decisionState:x.decisionState,identityMatched:x.identityMatched,fresh:x.fresh,publishedAt:x.publishedAt,ageMinutes:x.ageMinutes,newsScore:x.newsScore,confidence:x.confidence,sources:x.sources,sourceCount:x.sourceCount,allSourceCount:x.allSourceCount,clusteredStories:x.clusteredStories,priority:x.priority,held:x.held,urgent:x.urgent,positive:x.positive,negative:x.negative,positiveConfirmed:x.positiveConfirmed,negativeConfirmed:x.negativeConfirmed,chaseRisk:x.chaseRisk,error:x.error||null})).slice(0,10)};
    s.paperStartPersistence={...this.__startPersistenceV31711,idempotentStart:true,explicitResetRequiredForNewRun:true};
    s.executionModel={...(s.executionModel||{}),freshNewsCatalystV31710:true,urgentNewsFirstWithinFixedBudget:true,newsEventClustering:true,regionalNewsReactionLearning:true,newsReactionWindowsMinutes:[15,60,240,360],newsRequiresMarketReaction:true,newsAloneCannotBuy:true,criticalStructuralNegativeCanExitBeforeChartConfirmation:true,negativeCatalystExitConfirmation:true,idempotentPaperStartV31711:true};
