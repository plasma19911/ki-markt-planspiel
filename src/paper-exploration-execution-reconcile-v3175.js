...
export async function reconcilePaperExplorationExecutionV3175({engine,unified,brokerRows=[],baseResult=null,now=Date.now()}={}){
  const diag={version:31.75,checkedAt:new Date(now).toISOString(),attempted:false,executed:false,reason:null,symbol:null,allocationPct:null,amount:null};
  if(PAPER_EXPLORATION_EXECUTION_RECONCILE_V3175.enabled!==true||!engine?.store?.update||!unified?.latest?.plan)return{...diag,reason:'UNAVAILABLE'};
  ...
  if(num(buy?.confidence)<PAPER_EXPLORATION_EXECUTION_RECONCILE_V3175.minConfidence)return{...diag,reason:'LOW_PLAN_CONFIDENCE'};
  ...
  try{
    const saved=await engine.store.update(s=>{
      if(arr(s?.positions).length)return{executed:false,reason:'POSITION_ALREADY_OPEN'};
      const c=arr(s?.candidates).find(x=>key(x)===diag.symbol);if(!c)return{executed:false,reason:'CANDIDATE_NOT_IN_CURRENT_STATE'};
      if(!candidateFresh(c,now))return{executed:false,reason:'CANDIDATE_NOT_FRESH',fresh:c?.fresh??null,updatedAt:c?.updated_at||null};
      ...
      const budget=Math.min(cash,cash*pct/100),notional=Math.max(0,(budget-feeFixed)/(1+feeRate)),fee=feeFixed+notional*feeRate;
      ...
      s.positions=arr(s.positions);s.positions.push({symbol:diag.symbol,name:c?.name||broker?.tradeRepublicName||diag.symbol,instrument_type:c?.instrument_type||'EQUITY',theme:c?.theme||null,company_key:c?.company_key||null,invested:notional,entry_fee:fee,entry_price:exec,last_price:price,entry_fx:fx,last_fx:fx,fx_verified:true,currency:c?.currency||base,quote_currency_raw:c?.quote_currency_raw||c?.currency||base,quote_price_scale:num(c?.quote_price_scale,1),quote_unit_normalized:Boolean(c?.quote_unit_normalized),opened_at:openedAt,last_added_at:null,add_count:0,score:num(c?.score),signal_confidence:num(buy?.confidence),paper_exploration_v3175:true,broker_isin:broker?.isin||null});
      const eq=equity(s),id=nextId(s.history);s.history=arr(s.history);s.history.push({id,ts:openedAt,end_ts:null,event_count:1,start_scan:num(s?.config?.scan_count)+1,end_scan:num(s?.config?.scan_count)+1,action:'KAUF',symbol:diag.symbol,name:c?.name||diag.symbol,instrument_type:c?.instrument_type||'EQUITY',amount:-(notional+fee),fee,trade_pnl:null,cash_before:before,cash_after:s.config.cash,equity:eq,total_pnl:eq-num(s?.config?.start_capital),score:num(c?.score),reason:`V31.7.5 PAPER-EXECUTION-RECONCILE: kontrollierter Probe-BUY aus UnifiedDecisionCore nach verlorenem Basis-Execution-Pfad. ${String(buy?.reason||'').slice(0,500)} · Order ${notional.toFixed(2)} ${base} · Slippage ${slip.toFixed(2)}%`});
      if(s.history.length>800)s.history=s.history.slice(-800);
      s.aiLog=arr(s.aiLog);s.aiLog.push({id:nextId(s.aiLog),ts:openedAt,kind:'TRADE',symbol:diag.symbol,title:'Probe-Kauf ausgeführt',message:`${diag.symbol}: kontrollierter Paper-Probe-BUY wurde nach erneuter Fresh/FX/Broker-Prüfung ins Paper-Depot reconciled.`,confidence:num(buy?.confidence),meta:{paperExplorationExecutionReconcileV3175:true,allocationPct:pct,amount:notional,isin:broker?.isin||null}});if(s.aiLog.length>300)s.aiLog=s.aiLog.slice(-300);
      return{executed:true,reason:'CONTROLLED_PROBE_RECONCILED',symbol:diag.symbol,amount:notional,allocationPct:pct,equity:eq,cash:s.config.cash};
    });
