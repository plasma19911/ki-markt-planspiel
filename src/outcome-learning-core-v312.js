...
function learningProfile(m,now=Date.now()){
  ...
  const buyWins=buys.filter(x=>num(x.returnPct)>=.12).length,buyHitRate=buys.length?buyWins/buys.length:null,avgBuy=weightedAverage(buys,'returnPct',now),avgAll=weightedAverage(recent,'returnPct',now);
  const missed=holds.filter(x=>num(x.returnPct)>=.5).length,badBuys=buys.filter(x=>num(x.returnPct)<=-.25).length,earlySells=sells.filter(x=>num(x.returnPct)>=.5).length,correctSells=sells.filter(x=>num(x.returnPct)<=-.25).length;
  let thresholdAdjustment=0,allocationAdjustment=0,mode='WARMUP';
  if(matured>=OUTCOME_LEARNING_V312.minLearningSamples){
    mode='BALANCED';
    const badBuyRate=buys.length?badBuys/buys.length:0,missRate=holds.length?missed/holds.length:0;
    if(buys.length>=5&&((buyHitRate??0)<.43||(avgBuy??0)<-.12||badBuyRate>.4)){thresholdAdjustment=3;allocationAdjustment=-6;mode='DEFENSIVE'}
    else if(buys.length>=5&&((buyHitRate??0)<.5||(avgBuy??0)<0)){thresholdAdjustment=1.5;allocationAdjustment=-3;mode='CAUTIOUS'}
    else if(missRate>.28&&(buyHitRate==null||buyHitRate>=.5)){thresholdAdjustment=-2;allocationAdjustment=4;mode='OPPORTUNITY'}
    else if(buys.length>=5&&(buyHitRate??0)>=.62&&(avgBuy??0)>=.15){thresholdAdjustment=-2;allocationAdjustment=6;mode='CONFIDENT'}
    else if(missRate>.16){thresholdAdjustment=-1;allocationAdjustment=2;mode='PROACTIVE'}
  }
  return{mode,matured,buySamples:buys.length,buyHitRate:buyHitRate==null?null:+(buyHitRate*100).toFixed(1),avgBuy20mReturnPct:avgBuy==null?null:+avgBuy.toFixed(3),avg20mReturnPct:avgAll==null?null:+avgAll.toFixed(3),missedOpportunities:missed,badBuys,earlySells,correctSells,thresholdAdjustment,allocationAdjustment};
}
function groupUpdate(bucket,name,ret){if(!name)return;const r=bucket[name]&&typeof bucket[name]==='object'?bucket[name]:{n:0,sum:0,wins:0};r.n++;r.sum+=ret;if(ret>=.12)r.wins++;r.avg=+(r.sum/r.n).toFixed(4);r.hitRate=+(r.wins/r.n*100).toFixed(1);bucket[name]=r}
...
function evaluateSamples(m,observations,now){
  for(const [symbol,slot] of Object.entries(m.symbols)){
    ...
    for(const sample of samples){if(!(num(sample.price)>0))continue;sample.evaluations=sample.evaluations&&typeof sample.evaluations==='object'?sample.evaluations:{};
      for(const h of OUTCOME_LEARNING_V312.horizonsMinutes){if(sample.evaluations[h])continue;const age=now-num(sample.ts),min=h*60000,max=(h+(h>=240?120:h>=60?45:20))*60000;if(age<min||age>max)continue;const ret=(obs.price/num(sample.price)-1)*100;sample.evaluations[h]={ts:now,returnPct:+ret.toFixed(4)};
        if(h===20){const action=String(sample.action||'HOLD').toUpperCase(),row={ts:now,symbol,action,returnPct:+ret.toFixed(4),score:num(sample.score),forecast20mScore:num(sample.forecast20mScore),theme:sample.theme||'UNKNOWN',regime:sample.regime||'SIDEWAYS'};m.recent20.push(row);m.recent20=m.recent20.slice(-OUTCOME_LEARNING_V312.maxRecentOutcomes);m.stats.evaluated20++;if(action==='HOLD'&&ret>=.5)m.stats.missedOpportunities++;if(action==='BUY'&&ret<=-.25)m.stats.badBuys++;if(action==='SELL'&&ret>=.5)m.stats.earlySells++;if(action==='SELL'&&ret<=-.25)m.stats.correctSells++;learnWeights(m,sample,ret);groupUpdate(m.groupStats.regime,sample.regime||'SIDEWAYS',ret);groupUpdate(m.groupStats.theme,sample.theme||'UNKNOWN',ret);for(const src of arr(sample.sources))groupUpdate(m.groupStats.source,src,ret)}
      }
    }
  }
}
...
export function updateOutcomeLearningMemoryV312(memory={},state={},now=Date.now()){
  ...
  return{memory:m,predictions,status:{enabled:true,...OUTCOME_LEARNING_V312,...profile,trackedSymbols:Object.keys(m.symbols).length,currentCandidates:Object.keys(predictions).length,weights:{...m.weights},topForecasts:top,recentMissedOpportunities:recentMisses,rule:'V31.2 verfolgt Kandidaten und Depotpositionen kontinuierlich. BUY/HOLD/SELL werden nach 5/20/60/240 Minuten mit dem echten späteren Kurs verglichen. Verpasste Chancen, Fehlkäufe und zu frühe Verkäufe fließen in Schwellen, Positionsgröße und die Gewichte für Score-Geschwindigkeit, Momentum, Beschleunigung, News, Konfidenz und Chart-Richtung ein. Neuere 20m-Ergebnisse werden stärker gewichtet.'}};
}

export function recordOutcomeDecisionsV312(memory={},state={},plan={},predictions={},now=Date.now()){
  const m=cleanMemory(memory),actions=new Map(arr(plan?.actions).map(a=>[key(a),String(a?.action||'HOLD').toUpperCase()])),rows=new Map();for(const c of candidateRows(state))rows.set(key(c),c);for(const p of arr(state?.positions))if(key(p))rows.set(key(p),p);
  for(const [symbol,c] of rows){const v=vector(c);if(!(v.price>0))continue;const p=predictions[symbol]||forecastFor(v,null,learningProfile(m,now),m.weights,now),action=['BUY','SELL','HOLD'].includes(actions.get(symbol))?actions.get(symbol):'HOLD',slot=m.symbols[symbol]&&typeof m.symbols[symbol]==='object'?m.symbols[symbol]:{samples:[]};let samples=arr(slot.samples).filter(x=>x&&Number.isFinite(Number(x.ts))&&now-num(x.ts)<36*60*60*1000),last=samples.at(-1),spacing=OUTCOME_LEARNING_V312.snapshotSpacingMinutes*60000;if(!last||now-num(last.ts)>=spacing||String(last.action)!==action){samples.push({ts:now,price:v.price,score:v.score,action,forecast20mScore:p.forecast20mScore,signalConfidence:p.signalConfidence,features:{...(p.features||featureVector(v,0))},earlySignal:p.earlySignal===true,theme:v.theme,regime:p.regime||regimeOf(v),sources:v.sources,evaluations:{}})}samples=samples.slice(-OUTCOME_LEARNING_V312.maxSamplesPerSymbol);m.symbols[symbol]={samples,lastForecast:p.forecast20mScore,lastScore:v.score,lastAction:action,lastSeenAt:now,theme:v.theme}}
