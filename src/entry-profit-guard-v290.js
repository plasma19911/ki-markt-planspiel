...
export function enforceEntryProfitV290(plan,state={},storage=null,now=Date.now()){
  ...
  for(const d of promotable){
    ...
    actions[idx]={...a,action:'BUY',allocation_pct:pct,confidence:clamp(Math.max(num(a?.confidence,.58),conf),.56,.90),reason:`ENTRY V29.0 ${d.tier}: ${d.symbol} Kaufscore ${d.score.toFixed(1)}/100 · Trend ${d.trend.deltaWindow>=0?'+':''}${d.trend.deltaWindow.toFixed(1)} · ${Math.round(d.coverage*100)}% Datenabdeckung · ${d.label}. Früh genug in bestätigte Starter einsteigen; niedrige Score-Zonen verlangen bewusst stärkere Marktbestätigung. Hard-Block, Falling-Knife, FOMO und Überdehnung bleiben gesperrt.`};
...
export class EntryProfitGuardV290{
  ...
  status(){const mem={...defaults(),...read(this.storage,defaults())};return{enabled:true,version:29.0,thresholds:ENTRY_PROFIT_V290,latest:this.latest?.counters||null,candidateBehaviors:this.latest?.candidateBehaviors||[],profitBehaviors:this.latest?.profitBehaviors||[],stats:mem.stats||{},rule:'V29.0 Einstieg: 50–52 beobachten, 53–55 Scout nur mit sehr hoher Datenabdeckung und starker Beschleunigung, 56–57 Mikro, 58–61 früh, 62+ regulär. Je niedriger der Score, desto stärker muss Momentum/Katalysator bestätigen. Gewinner können bei nachlassendem Peak-/Momentum-Verhalten auch bei Haltescore 70–75 gesichert werden; bei weiter steigendem Trend bleibt HOLD.'}}
}
