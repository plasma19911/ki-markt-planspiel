...
export const UNIFIED_DECISION_CORE_V310={
  version:31.6,
  patch:'31.6-canonical-entry-score+31.5-evidence-fusion+31.4-shadow-calibration',
...
function actionSnapshot(a={}){return{symbol:key(a),action:String(a?.action||'HOLD').toUpperCase(),allocationPct:num(a?.allocation_pct),entryScoreV316:Number.isFinite(Number(a?.entryScoreV316))?num(a.entryScoreV316):null,dataQualityV316:Number.isFinite(Number(a?.dataQualityV316))?num(a.dataQualityV316):null,expectedNetEdgePctV316:Number.isFinite(Number(a?.expectedNetEdgePctV316))?num(a.expectedNetEdgePctV316):null,predictiveEntry:a?.predictiveEntryV311===true,outcomeEntry:a?.outcomeEntryV312===true,forecast20mScore:Number.isFinite(Number(a?.forecast20mScore))?num(a?.forecast20mScore):null,reason:String(a?.reason||'').slice(0,700)}}
...
function candidateDiagnostics(state={},shadowMem={},cost=.291){const universe=arr(state?.candidates);return universe.map(c=>{const a=canonicalEntryAssessmentV316(c,universe,arr(shadowMem?.matured),cost);return{symbol:key(c),score:a.score,dataQuality:a.dataQuality,expectedNetEdgePct:a.expectedNetEdgePct,legacyScore:canonicalScore(c?.daytradeLiveScore??c?.decisionScore??c?.score),m5:num(c?.momentum5Pct,c?.momentum5),m20:num(c?.momentum20Pct,c?.momentum20),brokerVerified:c?.brokerVerified===true,brokerMatchMode:c?.brokerMatchMode||null}}).filter(x=>x.symbol).sort((a,b)=>b.score-a.score).slice(0,16)}
...
export async function enforceUnifiedDecisionCoreV310(plan,state={},input=null,brokerRows=[],now=Date.now(),outcomeLearning=null,shadowStorage=null){
  ...
  const audit={
    ...
    ruleOrder:['hard safety from legacy core','V31.2 continuous outcome learning + early-entry','legacy opportunity proposal','V31.6 canonical entry score/data quality/net edge approval','V31.3 hard-stop/trailing/paired-rotation/stagnation/profit-fade/re-entry/sizing authority'],
    note:'V31.6 verwendet für die endgültige Kaufzulassung genau einen kanonischen Einstiegsscore je Symbol und Scan. Alte Zwischenwerte sind nur Diagnose. Fehlende Volumen-/Newsdaten senken die Datenqualität; neue V31.6-Samples kalibrieren die erwartete Nettorendite nach Kosten. SELL/HOLD und harte Safety bleiben getrennt.'
  };
  finalPlan.summary=`${String(finalPlan.summary||'').slice(0,145)} · V31.6 Unified+Canonical: ${changes.length} finale Änderung(en), Kaufsystem ${shadowPass.counters?.calibrated?'KALIBRIERT':'WARMUP'}.`;
