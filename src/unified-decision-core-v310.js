...
import {updateOutcomeLearningMemoryV312,recordOutcomeDecisionsV312,enforceOutcomeEarlyEntryV312,enforceOutcomeMissRecoveryV31751,OUTCOME_LEARNING_V312} from './outcome-learning-core-v312.js';
import {enforceShadowLearningV314,SHADOW_LEARNING_V314,canonicalEntryAssessmentV316} from './shadow-learning-v314.js';
import {enforcePaperExplorationV3172,PAPER_EXPLORATION_V3172} from './paper-exploration-v3172.js';
...
export const UNIFIED_DECISION_CORE_V310={
  version:31.7,
  patch:'31.7.51-counterfactual-miss-recovery',
...
function learningAudit(status={}){return{version:status?.version||31.2,mode:status?.mode||'WARMUP',matured:num(status?.matured),buySamples:num(status?.buySamples),buyHitRate:status?.buyHitRate??null,avgBuy20mReturnPct:status?.avgBuy20mReturnPct??null,avg20mReturnPct:status?.avg20mReturnPct??null,missedOpportunities:num(status?.missedOpportunities),missedObservations:num(status?.missedObservations),actionableHoldSetups:num(status?.actionableHoldSetups),actionableMissRate:status?.actionableMissRate??null,avgMissedNetReturnPct:status?.avgMissedNetReturnPct??null,missRecoveryActive:status?.missRecoveryActive===true,badBuys:num(status?.badBuys),earlySells:num(status?.earlySells),correctSells:num(status?.correctSells),trackedSymbols:num(status?.trackedSymbols),currentCandidates:num(status?.currentCandidates),thresholdAdjustment:num(status?.thresholdAdjustment),allocationAdjustment:num(status?.allocationAdjustment),weights:status?.weights||null}}
...
export async function enforceUnifiedDecisionCoreV310(plan,state={},input=null,brokerRows=[],now=Date.now(),outcomeLearning=null,shadowStorage=null){
  ...
  const firstShadowPass=await enforceShadowLearningV314(buyPass.plan,state,shadowStorage,now);
  const missPass=enforceOutcomeMissRecoveryV31751(firstShadowPass.plan,state,outcomeLearning||{},brokerRows,firstShadowPass.mem,now);
  const shadowPass=missPass.counters?.injected?await enforceShadowLearningV314(missPass.plan,state,shadowStorage,now):{...firstShadowPass,plan:missPass.plan};
  const expectancyPass=enforceExpectancyCoreV310(shadowPass.plan,state,now);
  const explorationPass=enforcePaperExplorationV3172(expectancyPass.plan,state,outcomeLearning||{},brokerRows,now);
  const finalPlan=explorationPass.plan;
  ...
  const audit={
    ...
    changes,
    counters:{outcome:predictivePass.counters||{},predictive:predictivePass.counters||{},capital:buyPass.counters||{},missRecovery:missPass.counters||{},shadow:shadowPass.counters||{},expectancy:expectancyPass.counters||{},exploration:explorationPass.counters||{}},
    shadowLearning:{...shadowPass.counters,calibration:shadowPass.calibration?.calibration||[]},
    ruleOrder:['hard safety from legacy core','V31.2 continuous outcome learning + early-entry','legacy opportunity proposal','V31.7 canonical score + orthogonal confirmation + cost-aware probation approval','V31.7.51 unique tradeable missed-opportunity recovery','V31.7 canonical safety revalidation','V31.7 hard-stop/trailing/failed-setup/rotation/stagnation/profit-fade authority','controlled paper exploration'],
    note:'V31.7.51 zaehlt verpasste Chancen nur einmal je handelbarem Setup und nach realistischen Kosten. Erst belastbare 20m/60m-Gegenfakten duerfen einen wirtschaftlich grossen Recovery-BUY vorschlagen; der Vorschlag wird danach erneut von der kanonischen Shadow-, News-, Klumpen- und Abstandspruefung validiert.'
  };
  ...
  finalPlan.summary=`V31.7.51 FINAL: ${counts.buy} BUY · ${counts.sell} SELL · ${counts.hold} HOLD · Score 60 + Datenqualität 55 + unabhängiger Beleg · ${changes.length} finale Änderung(en) · ${arr(state?.candidates).length} aktuelle Lernkandidaten · Miss-Recovery ${missPass.counters?.injected?'geprüft':'nein'} · Probe ${explorationPass.counters?.injected?'AKTIV':'nein'}.`;
  return{plan:finalPlan,audit,counterfactuals:missPass.counterfactuals||{},counters:{changes:changes.length,outcome:predictivePass.counters||{},predictive:predictivePass.counters||{},capital:buyPass.counters||{},missRecovery:missPass.counters||{},shadow:shadowPass.counters||{},expectancy:expectancyPass.counters||{},exploration:explorationPass.counters||{}}};
}
...
export class UnifiedDecisionCoreV310{
  ...
  async run(model,input){
    ...
    try{
      const recorded=recordOutcomeDecisionsV312(outcomeLearning.memory||{},state,out.plan,outcomeLearning.predictions||{},now,out.counterfactuals||{});this.predictiveStatus={...outcomeLearning.status,...recorded.status};out.audit.outcomeLearning=learningAudit(this.predictiveStatus);out.audit.predictiveLearning=out.audit.outcomeLearning;
