...
export const UNIFIED_DECISION_CORE_V310={
  version:31.7,
  patch:'31.7.2-candidate-state-recovery+orthogonal-confirmation+probation+failed-setup-exit+controlled-paper-exploration',
...
export async function enforceUnifiedDecisionCoreV310(plan,state={},input=null,brokerRows=[],now=Date.now(),outcomeLearning=null,shadowStorage=null){
  ...
  const audit={
    ...
    ruleOrder:['hard safety from legacy core','V31.2 continuous outcome learning + early-entry','legacy opportunity proposal','V31.7 canonical score + orthogonal confirmation + probation approval','V31.7 hard-stop/trailing/failed-setup/rotation/stagnation/profit-fade authority','V31.7.2 controlled paper exploration only when zero BUY samples persist after substantial learning'],
    note:'V31.7.2 behält die normalen Kaufregeln unverändert bei. Nur im Planspiel-Deadlock mit vielen ausgewerteten Fällen, mehreren verpassten Chancen und weiterhin null BUY-Samples darf eine einzelne kleine, exakt Trade-Republic-verifizierte Lernposition auf einen frischen positiv bestätigten Kandidaten eröffnet werden. Harte Markt-, News-, Quote- und Re-Entry-Sperren bleiben bindend.'
  };
  finalPlan.summary=`${String(finalPlan.summary||'').slice(0,128)} · V31.7.2 Unified: ${changes.length} finale Änderung(en), ${arr(state?.candidates).length} aktuelle Lernkandidaten, Probe ${explorationPass.counters?.injected?'AKTIV':'nein'}.`;
