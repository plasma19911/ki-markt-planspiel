...
export const UNIFIED_DECISION_CORE_V310={
  version:31.7,
  patch:'31.7-orthogonal-confirmation+probation+failed-setup-exit',
...
export async function enforceUnifiedDecisionCoreV310(plan,state={},input=null,brokerRows=[],now=Date.now(),outcomeLearning=null,shadowStorage=null){
  ...
  const audit={
    ...
    note:'V31.7 lässt korreliertes Trend- und Relative-Stärke-Momentum ohne positives Volumen- oder News-Signal nicht mehr als starken Kauf erscheinen. Negative kompatible Warmup-Ergebnisse pausieren den betroffenen Scorebereich, wiederholte BUYs auf bereits gehaltene Aktien werden blockiert und gescheiterte Setups können nach 90 Minuten nur mit mehrfach bestätigter Schwäche beendet werden.'
  };
  finalPlan.summary=`${String(finalPlan.summary||'').slice(0,138)} · V31.7 Unified: ${changes.length} finale Änderung(en), Kaufsystem ${shadowPass.counters?.calibrated?'KALIBRIERT':'WARMUP'}.`;
