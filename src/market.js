...
async function deepChart(info){const u=new URL(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(info.symbol)}`);u.searchParams.set('range','1d');u.searchParams.set('interval','1m');u.searchParams.set('includePrePost','false');const r=await fetch(u,{headers});if(!r.ok)return null;const j=await r.json(),res=j?.chart?.result?.[0];if(!res)return null;const q=res?.indicators?.quote?.[0]||{},cl=(q.close||[]).filter(v=>Number.isFinite(Number(v))).map(Number),vol=(q.volume||[]).map(v=>num(v));if(cl.length<22)return null;const price=cl.at(-1),e9=ema(cl,9),e21=ema(cl,21),rr=rsi(cl),m5=(price/cl.at(-6)-1)*100,m20=(price/cl.at(-21)-1)*100,pclose=num(res.meta?.previousClose,cl[0]),day=pclose?(price/pclose-1)*100:0,vbase=vol.slice(-21,-1).filter(x=>x>0),vavg=vbase.length?vbase.reduce((a,b)=>a+b,0)/vbase.length:0,vr=vavg?num(vol.at(-1))/vavg:1,last=(res.timestamp||[]).at(-1)||res.meta?.regularMarketTime||0,fresh=Date.now()/1000-last<35*60;let score=0,reason=[];if(e9&&e21&&e9>e21){score+=1.7;reason.push('EMA9 > EMA21')}else{score-=1;reason.push('EMA-Trend schwach')}if(e21&&price>e21)score+=.8;else score-=.6;if(rr!==null){if(rr>=48&&rr<=68){score+=1;reason.push(`RSI ${rr.toFixed(0)}`)}else if(rr>=78){score-=1.5;reason.push(`RSI ${rr.toFixed(0)} ueberhitzt`)}else if(rr<=32)score-=.8}if(m5>.18){score+=.8;reason.push(`5m +${m5.toFixed(2)}%`)}else if(m5<-.25)score-=.9;if(m20>.5){score+=1.2;reason.push(`20m +${m20.toFixed(2)}%`)}else if(m20<-.5)score-=1.2;if(vr>1.5){score+=.7;reason.push(`Volumen x${vr.toFixed(1)}`)}if(day>1)score+=.4;if(day<-1)score-=.5;return{...info,price,score,dayChange:day,momentum5:m5,momentum20:m20,rsi:rr,volumeRatio:vr,newsScore:0,newsConfidence:0,newsSources:[],fresh,reasons:reason,headlines:[]}}
