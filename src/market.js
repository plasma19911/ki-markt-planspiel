...
function newsAgeHours(ts){if(!ts)return Infinity;const parsed=Date.parse(ts);if(!Number.isFinite(parsed))return Infinity;return Math.max(0,(Date.now()-parsed)/3600000)}
// Frische Meldungen dominieren bewusst: aeltere Headlines verlieren sehr schnell an Einfluss,
// weil sie wahrscheinlicher bereits in vorherige Kurse/Markterwartungen eingeflossen sind.
function recencyWeight(ts){
 const h=newsAgeHours(ts);if(!Number.isFinite(h))return .06;
 if(h<=.25)return 1.75;   // <= 15 Min: Breaking News
 if(h<=1)return 1.55;     // <= 1 Std
 if(h<=3)return 1.30;     // <= 3 Std
 if(h<=6)return 1.00;     // <= 6 Std
 if(h<=12)return .62;
 if(h<=18)return .35;
 if(h<=24)return .16;
 if(h<=36)return .05;
...
function marketOpen(info,date=new Date()){const [,tz,open,close,label]=sessionRule(info),p=localParts(date,tz),days=['Mon','Tue','Wed','Thu','Fri'];if(!days.includes(p.weekday))return{open:false,label};const mins=num(p.hour)*60+num(p.minute);return{open:mins>=open&&mins<close,label}}
...
export async function loadUniverse(env,cfg){
 ...
 all.push(...PRIORITY_EQUITIES);if(cfg.include_etfs)all.push(...CORE_ETFS);if(cfg.include_leverage)all.push(...LEVERAGED_ETFS);
...
async function sparkBatch(symbols,lookup){const u=new URL('https://query1.finance.yahoo.com/v7/finance/spark');u.searchParams.set('symbols',symbols.join(','));u.searchParams.set('range','1d');u.searchParams.set('interval','5m');u.searchParams.set('indicators','close');u.searchParams.set('includePrePost','false');const r=await fetch(u,{headers});if(!r.ok)throw new Error(`Yahoo Spark ${r.status}`);const j=await r.json(),out=[];for(const item of j?.spark?.result||[]){const res=item?.response?.[0];if(!res)continue;const m=res.meta||{},sym=String(item.symbol||m.symbol||'').toUpperCase(),info=lookup.get(sym);if(!info)continue;const c=(res?.indicators?.quote?.[0]?.close||[]).filter(v=>Number.isFinite(Number(v))).map(Number);if(c.length<3)continue;const price=num(m.regularMarketPrice,c.at(-1)),prev=num(m.previousClose,c[0]),day=prev?(price/prev-1)*100:0,back=c[Math.max(0,c.length-4)],mom=back?(price/back-1)*100:0,age=m.regularMarketTime?Date.now()/1000-num(m.regularMarketTime):999999;out.push({...info,price,dayChange:day,preScore:day*.65+mom*1.35,fresh:age<35*60})}return out}
async function deepChart(info){const u=new URL(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(info.symbol)}`);u.searchParams.set('range','1d');u.searchParams.set('interval','1m');u.searchParams.set('includePrePost','false');const r=await fetch(u,{headers});if(!r.ok)return null;const j=await r.json(),res=j?.chart?.result?.[0];if(!res)return null;const q=res?.indicators?.quote?.[0]||{},cl=(q.close||[]).filter(v=>Number.isFinite(Number(v))).map(Number),vol=(q.volume||[]).map(v=>num(v));if(cl.length<22)return null;const price=cl.at(-1),e9=ema(cl,9),e21=ema(cl,21),rr=rsi(cl),m5=(price/cl.at(-6)-1)*100,m20=(price/cl.at(-21)-1)*100,pclose=num(res.meta?.previousClose,cl[0]),day=pclose?(price/pclose-1)*100:0,vbase=vol.slice(-21,-1).filter(x=>x>0),vavg=vbase.length?vbase.reduce((a,b)=>a+b,0)/vbase.length:0,lastVol=num(vol.at(-1)),previousVol=num(vol.at(-2)),vr=vavg>0&&lastVol>0?lastVol/vavg:vavg>0&&previousVol>0?previousVol/vavg:1,last=(res.timestamp||[]).at(-1)||res.meta?.regularMarketTime||0,fresh=Date.now()/1000-last<35*60;let score=0,reason=[];if(e9&&e21&&e9>e21){score+=1.7;reason.push('EMA9 > EMA21')}else{score-=1;reason.push('EMA-Trend schwach')}if(e21&&price>e21)score+=.8;else score-=.6;if(rr!==null){if(rr>=48&&rr<=68){score+=1;reason.push(`RSI ${rr.toFixed(0)}`)}else if(rr>=78){score-=1.5;reason.push(`RSI ${rr.toFixed(0)} ueberhitzt`)}else if(rr<=32)score-=.8}if(m5>.18){score+=.8;reason.push(`5m +${m5.toFixed(2)}%`)}else if(m5<-.25)score-=.9;if(m20>.5){score+=1.2;reason.push(`20m +${m20.toFixed(2)}%`)}else if(m20<-.5)score-=1.2;if(vr>1.5){score+=.7;reason.push(`Volumen x${vr.toFixed(1)}`)}if(day>1)score+=.4;if(day<-1)score-=.5;return{...info,price,score,dayChange:day,momentum5:m5,momentum20:m20,rsi:rr,volumeRatio:vr,newsScore:0,newsConfidence:0,newsSources:[],fresh,reasons:reason,headlines:[]}}
...
function aggregateNews(...sets){
 ...
 const latestAt=items[0]?.publishedAt||null,latestWeight=recencyWeight(latestAt);
 const confidence=clamp((items.length?0.22:0)+Math.min(.28,items.length*.035)+Math.min(.28,sourceSet.size*.1)+Math.min(.22,latestWeight*.13),0,1);
 const freshImpact=Math.abs(score)*confidence*latestWeight;
 return{score,confidence,freshImpact,latestWeight,headlines:items.slice(0,8).map(x=>x.title),latestAt,sources:[...sourceSet],items};
}
...
export async function scanMarket(env,cfg,heldSymbols=[]){
 ...
 const enhancedTargets=newsOnly?radarTargets:[...deep.slice(0,2),...radarTargets.filter(x=>x.priority).slice(0,1)],enhanced=[];for(const c of enhancedTargets)if(c&&!enhanced.some(x=>x.symbol===c.symbol))enhanced.push(c);
 ...
 for(const c of deep){const n=nmap.get(c.symbol)||aggregateNews();c.newsScore=num(n.score);c.newsConfidence=num(n.confidence);c.newsSources=n.sources||[];c.headlines=n.headlines||[];c.score+=c.newsScore*c.newsConfidence*n.latestWeight;if(c.newsScore>.25)c.reasons.push(`News +${c.newsScore.toFixed(1)} · frisch ${n.latestWeight.toFixed(2)} (${c.newsSources.join('+')||'Quelle'})`);if(c.newsScore<-.25)c.reasons.push(`News ${c.newsScore.toFixed(1)} · frisch ${n.latestWeight.toFixed(2)} (${c.newsSources.join('+')||'Quelle'})`);if(c.type==='LEVERAGED_ETF')c.score-=.25}
 deep.sort((a,b)=>b.score-a.score);

 const newsRadar=radarTargets.map(c=>{const n=nmap.get(c.symbol)||aggregateNews();const score=num(n.score),tendency=score>.28?'BULLISH':score<-.28?'BEARISH':'NEUTRAL';return{symbol:c.symbol,name:c.name||c.symbol,type:c.type,theme:c.theme||null,score,confidence:n.confidence,freshImpact:n.freshImpact,latestWeight:n.latestWeight,tendency,sourceCount:(n.sources||[]).length,sources:n.sources||[],headline:n.headlines?.[0]||'',headlines:n.headlines||[],newsAt:n.latestAt||null}}).filter(x=>x.headline).sort((a,b)=>b.freshImpact-a.freshImpact||(Date.parse(b.newsAt||0)||0)-(Date.parse(a.newsAt||0)||0));
