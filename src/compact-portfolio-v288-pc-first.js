...
export class MarketPortfolio extends BasePortfolio{
  ...
  async status(){
    ...
    s.pcFirstScannerPolicy={enabled:true,version:Math.max(29.2,num(pc?.version,29.2)),runtimeVersion:'V31.7.30',displayVersion:`V31.7.30 · PC-Scanner-Kern ${Math.max(29.2,num(pc?.version,29.2)).toFixed(1)}`,mode:'PC-FIRST: kompletter Aktien-Master → jeder empfangene Wert 0–100 Vorscore → Top 400 → Deep 240 → Finalisten 60 → Cloudflare validiert breiteren Final-Slice (36 + 8 Forward)',masterUniverseCount,prescannedCount,validQuoteCount,preScoredCount,lastKnownCounts:historicalCounts,allReceivedRowsPreScored:pcFresh&&Boolean(pc?.allReceivedRowsPreScored??(preScoredCount>=validQuoteCount&&validQuoteCount>0)),stage2Count:pcFresh?num(pc?.stage2Count):0,deepCount:pcFresh?num(pc?.deepCount):0,finalistCount:pcFresh?num(pc?.finalistCount):0,fullCycleCoveragePct:pcFresh?num(pc?.fullCycleCoveragePct):0,targetFullCycleMinutes:num(pc?.targetFullCycleMinutes,s?.pcWideSweep?.fullMasterCycleMinutes||1),lastFullSweepAt:pc?.lastFullSweepAt||null,lastMinuteRefreshAt:pc?.lastMinuteRefreshAt||pc?.updatedAt||null,pcDataAgeSeconds:age,pcAgentOnline:online,pcDataFresh:pcFresh,cloudflareFallbackActive:!(online&&pcFresh),cloudflareValidationTarget:CF_VALIDATION_TARGET,cloudflareForwardReserve:CF_FORWARD_RESERVE,batchRequestsLastMinute:pcFresh?num(pc?.batchRequests):0,batchErrorsLastMinute:pcFresh?num(pc?.batchErrors):0,topPcCandidates:pcFresh?arr(pc?.candidates).slice(0,30):[],source:pc?.source||s?.pcWideSweep?.profile||null,usesExistingCsharpWideSweep:Boolean(pc?.mode==='CSHARP_PC_FIRST_FULL_MASTER'),scorePipeline:'ALL_RECEIVED_PRE_SCORE_0_100 → TOP400 → DEEP240 → FINAL60 → CLOUDFLARE36 + POSITIONS',fullResearchAppliedToFinalists:true,yahooBatchRecovery:Boolean(num(pc?.version)>=29.3),rule:'Nur frische PC-Zähler und Kandidaten gelten als live. Yahoo-Batches nutzen ab Scanner-Kern 29.3 zwei Hosts mit begrenztem Retry. Veraltete Werte bleiben getrennt als lastKnownCounts diagnostizierbar, dürfen aber weder Sichtfeld noch Pipeline-Gesundheit vortäuschen.'};
