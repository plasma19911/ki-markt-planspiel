...
function dashboardView(status={}){const out={};for(const k of DASHBOARD_FIELDS)if(status&&k in status)out[k]=status[k];const session=status?.tradeRepublicSession||status?.gettexSession||tradeRepublicSessionState(new Date());out.tradeRepublicSession={...session,broker:'Trade Republic',venue:'Bestpreis / Trade Republic execution',openLocal:'07:30',closeLocal:'23:00'};out.gettexSession=out.tradeRepublicSession;out.brokerTarget={...(status?.brokerTarget||{}),id:'TRADE_REPUBLIC',name:'Trade Republic',venue:'Bestpreis',weekdayHours:'07:30–23:00',stocksOnly:true,assetClass:'EQUITY_ONLY',regularOrderFeeEur:1,exactCatalogSynced:true};out.researchSignalFusionPolicy=addPositionScores(status);const raw=status?.dayReplayLearning;if(raw){const report=raw.report||raw;out.dayReplayLearning={report:{status:report?.status??null,processed:report?.processed??null,summary:report?.summary??raw?.summary??null}}};if(Array.isArray(out.history)){out.historyTotal=out.history.length;out.history=out.history.slice(0,60);out.historyWindow=60};if(Array.isArray(out.aiLog)){out.aiLogTotal=out.aiLog.length;out.aiLog=out.aiLog.slice(0,40);out.aiLogWindow=40};return out}
