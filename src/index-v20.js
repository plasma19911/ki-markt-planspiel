...
function dashboardView(status={}){const out={};for(const k of DASHBOARD_FIELDS)if(status&&k in status)out[k]=status[k];out.researchSignalFusionPolicy=addPositionScores(status);if(out.profitExitPolicy){const {audit:_audit,...lightProfitExit}=out.profitExitPolicy;out.profitExitPolicy={...lightProfitExit,auditAvailableInFullStatus:true}}const raw=status?.dayReplayLearning;if(raw){const report=raw.report||raw;out.dayReplayLearning={hourly:raw.hourly||null,schedule:raw.schedule||null,report:{status:report?.status??null,processed:report?.processed??null,total:report?.total??null,updatedAt:report?.updatedAt??null,summary:report?.summary??raw?.summary??null}}};if(Array.isArray(out.history)){out.historyTotal=out.history.length;out.history=out.history.slice(0,60);out.historyWindow=60};if(Array.isArray(out.aiLog)){out.aiLogTotal=out.aiLog.length;out.aiLog=out.aiLog.slice(0,40);out.aiLogWindow=40};out.newsRadarVisible=arr(out.newsRadar).length;out.newsLearningTotal=num(out.newsLearning?.summary?.totalEvents,out.newsLearning?.summary?.pendingEvents);return out}
