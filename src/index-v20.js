...
const DASHBOARD_FIELDS=['runtimeVersion','liveDecisionVersion','predictiveLearningVersion','outcomeLearningVersion','unifiedDecisionCorePolicy','predictiveLearningPolicy','outcomeLearningPolicy','expectancyCorePolicy','manualTradePolicy','architecture','config','equity','pnl','pnl_pct','positions','history','snapshots','candidates','newsRadar','newsCatalystPolicy','sourceHealth','aiLog','statistics','risk','executionModel','futureWatch','marketRegime','investmentDossiers','intelligenceUpdatedAt','intelligenceModel','analysisNotice','pcAgent','pcAgentScanRecovery','pcFirstScannerPolicy','scannerScorePipelinePolicy','gettexSession','orderApproval','accounting','researchSignalFusionPolicy','comprehensiveOpportunityPolicy','calibratedActionScorePolicy','scannerBreadthPolicy','scoreHysteresisPolicy','entryProfitPolicy','dynamicProfitLockPolicy','profitExitPolicy','canonicalScorePolicy','finalDecisionPolicy','daytradeLargeCapPolicy','daytradeDipPolicy','daytradeEntryPolicy','daytradeLiveFeedbackPolicy','systemValidationPolicy'];
...
function dashboardView(status={}){const out={};for(const k of DASHBOARD_FIELDS)if(status&&k in status)out[k]=status[k];out.researchSignalFusionPolicy=addPositionScores(status);const raw=status?.dayReplayLearning;if(raw){const report=raw.report||raw;out.dayReplayLearning={hourly:raw.hourly||null,schedule:raw.schedule||null,report:{status:report?.status??null,processed:report?.processed??null,total:report?.total??null,updatedAt:report?.updatedAt??null,summary:report?.summary??raw?.summary??null}}};if(Array.isArray(out.history)){out.historyTotal=out.history.length;out.history=out.history.slice(0,60);out.historyWindow=60};if(Array.isArray(out.aiLog)){out.aiLogTotal=out.aiLog.length;out.aiLog=out.aiLog.slice(0,40);out.aiLogWindow=40};return out}
