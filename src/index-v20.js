...
export default{
 async fetch(request,env,ctx){
  ...
  if(u.pathname==='/api/agent/universe'&&request.method==='POST'){
   ...
   j.scannerProfile={version:29.3,mode:'PC_FIRST_FULL_MASTER_STAGED',batchSize:80,shards:4,targetFullMasterCycleMinutes:4,preScoreTarget:'ALL_RECEIVED_FRESH_ROWS',stage2Target:400,deepTarget:240,finalistTarget:60,cloudflareValidationTarget:18,yahooHosts:2,retriesPerHost:2,rule:'PC bewertet jeden empfangenen frischen Vollscan-Wert leicht mit 0–100 vor, danach Top400 → Deep240 → Final60. Yahoo-Batches wechseln bei Ausfall auf den zweiten Host und werden begrenzt wiederholt. Cloudflare erhält nur verdichtete Finalisten und bleibt Research-/Safety-/Paper-Ausführung plus Fallback.'};
   return Response.json(j,{headers:{'cache-control':'no-store','x-pc-scanner':'v29.3','x-pc-score-pipeline':'all-pre-score+top400+deep240+final60'}})
  }
