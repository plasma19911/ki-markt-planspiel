...
async function buildDashboardResponse(env){
 const status=await portfolio(env).dashboardStatus(),payload=dashboardView(status),generatedAt=Date.now();
 return Response.json(payload,{headers:{'cache-control':'private, no-cache','x-dashboard-generated-at':String(generatedAt),'x-planspiel-ui':'v31.7.45','x-worker-load':'dashboard-no-audit','x-scan-cadence':'cloudflare-minute-primary+optional-pc-acceleration','x-action-score':'decision-score-56-v30.3-system','x-entry-model':'v30.2-live-feedback+v30.1-fresh-tape+v30.0-dips','x-position-model':'v29.7-profit+v29.6-directional-held-score','x-profit-model':'adaptive-profit-v29.7','x-scanner-breadth':'worker-primary+optional-pc-full-master'}})
}
function clientDashboardResponse(response,state){const headers=new Headers(response.headers);headers.set('cache-control','private, no-cache');headers.set('x-dashboard-cache',state);return new Response(response.body,{status:response.status,headers})}
async function storeDashboard(cache,response){const headers=new Headers(response.headers);headers.set('cache-control','public, max-age=90');await cache.put(DASHBOARD_CACHE_KEY,new Response(response.body,{status:response.status,headers}))}
async function cachedDashboardResponse(env,ctx){
 const cache=globalThis.caches?.default;if(!cache)return buildDashboardResponse(env);
 const hit=await cache.match(DASHBOARD_CACHE_KEY);if(hit){const generated=Number(hit.headers.get('x-dashboard-generated-at')),age=Date.now()-generated;if(Number.isFinite(age)&&age<=DASHBOARD_STALE_MS){if(age>DASHBOARD_REFRESH_MS&&!dashboardRefresh){dashboardRefresh=buildDashboardResponse(env).then(async r=>{await storeDashboard(cache,r.clone());return r}).catch(e=>{console.error('Dashboard cache refresh failed',e);return null}).finally(()=>{dashboardRefresh=null});ctx?.waitUntil?.(dashboardRefresh)}return clientDashboardResponse(hit,'HIT')}}
 const fresh=await buildDashboardResponse(env);ctx?.waitUntil?.(storeDashboard(cache,fresh.clone()).catch(e=>console.error('Dashboard cache store failed',e)));return clientDashboardResponse(fresh,'MISS')
}
