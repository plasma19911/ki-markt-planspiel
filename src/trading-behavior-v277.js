...
function defaults(){return{version:1,signals:{},peaks:{},updatedAt:null}}
...
function netExit(position={},candidate={},config={}){
 const invested=Math.max(0,num(position?.invested)),entryFee=Math.max(0,num(position?.entry_fee)),price=num(candidate?.price??candidate?.last_price??position?.last_price),fx=num(candidate?.fxRate??candidate?.fx_rate??candidate?.last_fx??position?.last_fx??position?.entry_fx,0),qty=num(position?.zero_quantity,0),entryPrice=num(position?.entry_price),entryFx=num(position?.entry_fx,1);
 ...
 return{euro:net,pct:den>0?net/den*100:null,exitFee:fee};
}
...
export function enforceTradingBehaviorV277(plan,state={},storage=null,now=Date.now()){
 ...
 const counters={entryConfirmationWaits:0,lateImpulseBlocks:0,deteriorationBlocks:0,saturationBlocks:0,convictionSizeCaps:0,tinyProfitHolds:0};
 ...
 for(const raw of plan.actions){
  ...
  if(action==='BUY'){
   ...
   const c=candidates.get(s)||{},m=metrics(c),type=entryType(reason),sig=mem.signals[s],sigAge=sig?Math.max(0,(now-num(sig.at,now))/60000):Infinity,priceJump=sig&&sig.price>0&&m.price>0?(m.price/sig.price-1)*100:null;
   const remember=()=>{mem.signals[s]={at:now,type,family:family(type),score:m.score,confidence:m.confidence,price:m.price}};
   if(lateImpulse(m)){counters.lateImpulseBlocks++;remember();out.push({symbol:s,action:'HOLD',confidence:Math.max(.72,num(a?.confidence,.72)),allocation_pct:0,reason:`ENTRY-PATIENCE V27.7: ${s} ist im aktuellen 5-Minuten-Impuls zu schnell gelaufen (5m ${m.m5.toFixed(2)}%, Beschleunigung ${m.accel.toFixed(2)}). Kein FOMO-Kauf; erst Stabilisierung/Reclaim im nächsten Scan.`});continue}
   if(deteriorating(m)){counters.deteriorationBlocks++;remember();out.push({symbol:s,action:'HOLD',confidence:Math.max(.70,num(a?.confidence,.70)),allocation_pct:0,reason:`ENTRY-PATIENCE V27.7: Setup verschlechtert sich vor Ausführung (5m ${m.m5.toFixed(2)}%, 20m ${m.m20.toFixed(2)}%, Beschleunigung ${m.accel.toFixed(2)}). Kein Kauf in nachlassende Struktur.`});continue}
   if(pf.investedRatio>=.85&&!strongCandidate(m)){counters.saturationBlocks++;remember();out.push({symbol:s,action:'HOLD',confidence:Math.max(.70,num(a?.confidence,.70)),allocation_pct:0,reason:`CAPITAL-SELECTIVITY V27.7: Depot bereits ${(pf.investedRatio*100).toFixed(0)}% investiert. Restcash bleibt für überdurchschnittliche Setups reserviert; aktuelles Signal ist dafür nicht stark genug.`});continue}
   const confirmed=Boolean(sig&&sigAge>=2&&sigAge<=20&&sig.family===family(type)&&m.score>=Math.max(3.55,num(sig.score)-.5)&&m.confidence>=.52&&(priceJump===null||priceJump<=.80));
   if(!exceptional(m)&&!confirmed){counters.entryConfirmationWaits++;remember();const why=sig&&sigAge<=20&&priceJump!==null&&priceJump>.80?`Bestätigung kam erst nach weiterem +${priceJump.toFixed(2)}% Kursanstieg; nicht hinterherlaufen.`:'Normales Setup braucht zwei zeitlich getrennte bestätigte Scans.';out.push({symbol:s,action:'HOLD',confidence:Math.max(.68,num(a?.confidence,.68)),allocation_pct:0,reason:`ENTRY-CONFIRM V27.7: ${why}`});continue}
 ...
 // prune old confirmation memory
 for(const [s,v] of Object.entries(mem.signals))if(now-num(v?.at,0)>45*60000&&!positions.has(s))delete mem.signals[s];
 mem.version=1;mem.updatedAt=new Date(now).toISOString();write(storage,mem);
...
export class TradingBehaviorGuardV277{
 ...
 status(){return{enabled:true,version:27.7,latest:this.latest,entryConfirmation:true,lateImpulseRecheck:true,tinyProfitPatience:true,portfolioSaturationSelectivity:true,convictionSizing:true,rule:'Normale BUYs brauchen zeitlich getrennte Bestätigung; extreme 5m-Impulse werden nicht gejagt. Bei hoher Depotauslastung wird Restcash nur für starke Setups eingesetzt. PROFIT-EXITs mit Cent-Nettoertrag bleiben ohne echten Gewinnerbruch HOLD.'}}
}
