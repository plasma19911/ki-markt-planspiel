...
export class TradingBehaviorGuardV277{
 ...
 async run(model,input){const legacy=input===undefined&&model&&typeof model==='object',payload=legacy?model:input,r=legacy?await this.inner.run(model):await this.inner.run(model,input),plan=parsePlan(r);if(!plan)return r;const state=typeof this.getState==='function'?(this.getState()||{}):{},result=enforceTradingBehaviorV277(plan,state,this.storage,typeof this.now==='function'?this.now():Date.now());this.latest=result.counters;return encode(r,result.plan)}
 status(){return{enabled:true,version:27.7,stateVersion:2,latest:this.latest,entryConfirmation:true,minuteScanConfirmationSafe:true,exceptionalImmediateEntry:true,lateImpulseRecheck:true,tinyProfitPatience:true,heldFxFallbackSafe:true,portfolioSaturationSelectivity:true,convictionSizing:true,rule:'Normale BUYs brauchen zeitlich getrennte Bestätigung; 1-Minuten-Scans setzen den Bestätigungstimer nicht zurück. Außergewöhnlich starke, nicht überhitzte Setups dürfen nach allen Safety-/Anti-FOMO-Gates sofort einsteigen. Extreme 5m-Impulse werden nicht gejagt. PROFIT-EXITs mit Cent-Nettoertrag bleiben ohne echten Gewinnerbruch HOLD.'}}
}
