 ...
 async status(){
  ...
  if(s?.finalDecisionPolicy){
   s.finalDecisionPolicy={...s.finalDecisionPolicy,version:27.8,
    ...
    heldBuyPromptSuppression:true,softSellNeedsRepeatScan:true,timeAloneCannotCreateSell:true,hardRiskImmediate:true,severeStructureBreakImmediate:true,
  ...
  s.tradingBehaviorPolicy={...behavior,enabled:true,version:27.8,paperTradingOnly:true,
   ...
   note:'V27.8 blockiert Bestands-BUYs bereits vor der KI-Ausgabe, passt die Entry-Reife an die Volatilität an und verlangt für normale Soft-SELLs eine echte Folgescan-Bestätigung. Zeit allein ist niemals ein Exit-Grund.'};
  if(s?.executionModel)s.executionModel={...s.executionModel,finalDecisionControllerV278:true,tradingBehaviorV278:true,heldBuyPromptSuppression:true,adaptiveEntryConfirmation:true,softSellNeedsRepeatScan:true,timeAloneCannotCreateSell:true,minuteScanConfirmationSafe:true,lateImpulseRecheck:true,tinyProfitChurnBlocked:true,heldFxFallbackSafe:true,aiPlanJsonRepair:true,automaticScaleUp:false};
