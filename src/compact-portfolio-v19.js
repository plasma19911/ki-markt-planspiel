...
export class MarketPortfolio extends BasePortfolio{
 constructor(ctx,env){
  ...
  if(ai?.run&&!ai.__newsShockGuard){const wrapped=new NewsShockAiGuard(ai,{getState:()=>this._actualState?.()||{}});wrapped.__newsShockGuard=true;this.engine.env.AI=wrapped}
  ai=this.engine?.env?.AI;
 ...
 async status(){
  ...
  s.entryResearchPolicy={...(s.entryResearchPolicy||{}),dipFirstV2:true,dipSelectionMode:'RELATIVE_DYNAMIC',newsShockRetest:true,realDipMinDrawdown20mPct:null,realDipMaxPositiveDayPct:null,candleFlowConfirmation:true,multiTimeframeConfirmation:true,fixedDipPercentThreshold:false,realDipBeatsHighBuy:true};
  s.balancedAdaptive={...(s.balancedAdaptive||{}),exitTimingMode:'CANDLE_FLOW_DYNAMIC',softSellAbsoluteGraceMinutes:null,normalRotationMinAgeMinutes:null,quickProfitLockMinAgeMinutes:null,quickProfitLockPct:null,minimumNormalSoftExitProfitPct:null,lossRotationAllowed:'ONLY_WHEN_SELLER_FLOW_OR_HARD_RISK',dipFirstV2:true,newsShockRetest:true,candleFlowConfirmation:true,multiTimeframeConfirmation:true,alwaysInvested:false,cashAllowed:true};
  if(s.profitOptimizer)s.profitOptimizer={...s.profitOptimizer,profitFirst:false,fixedProfitTargets:false,minimumNormalSoftExitProfitPct:null,normalSoftExitMinAgeMinutes:null,quickProfitLockPct:null,quickProfitLockMinAgeMinutes:null,lossRotationAllowed:'CANDLE_CONFIRMED_ONLY',dipFirstV2:true,newsShockRetest:true,dipSelectionMode:'RELATIVE_DYNAMIC',realDipBeatsHighBuy:true,candleFlowConfirmation:true,multiTimeframeConfirmation:true,candleInterval:'1m',dailyContext:'6mo/1d',weeklyContext:'2y/1wk',foreignFxBuyRequiresRealFx:true,alwaysInvested:false,capitalInMotion:false,legacyFullCashFailsafe:'BLOCKED_BY_SELECTIVE_CAPITAL',timeBasedExit:false,maxSingleNewPositionRiskCapPct:28};
  if(s.executionModel)s.executionModel={...s.executionModel,profitFirst:false,fixedProfitTargets:false,normalSoftExitMinProfitPct:null,exitTiming:'CANDLE_FLOW_DYNAMIC',newsShockRetest:true,candleFlowConfirmation:true,multiTimeframeConfirmation:true,foreignFxBuyRequiresRealFx:true,cashMayRemain:true,fullCashPolicy:false,capitalInMotion:false,forcedMinimumBuyPct:null,timeBasedSell:false};
  return s;
 }
}
