...
export const HIGH_SCORE_CAPITAL_DEPLOYMENT_V309={
  version:30.9,
  patch:'30.9.2-winner-topup-and-100pct-dynamic',
  minScore:68,
  convictionScore:70,
  strongScore:75,
  veryStrongScore:80,
...
export function enforceHighScoreCapitalDeploymentV309(plan,state={},input=null,brokerRows=[]){
 ...
 if(positions.length<cst.maxOpenPositions){
   ...
   const heldRows=positions.map(p=>{const s=key(p),c=cmap.get(s)||{},i=idx.get(s),a=i===undefined?{}:actions[i],stable=num(p?.decisionScore,p?.score),raw=num(p?.rawDecisionScore,stable),pl=positionPnl(p),dir=String(p?.chartDirectionMode||'').toUpperCase();return{p,c,s,i,a,stable,raw,pl,dir,m5:m5(c),m20:m20(c),exact:brokerExact(c),hard:hard(a,c)}})
