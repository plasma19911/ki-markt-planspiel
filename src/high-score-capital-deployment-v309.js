import {canonicalEntryAssessmentV316,SHADOW_LEARNING_V314} from './shadow-learning-v314.js';
...
export const HIGH_SCORE_CAPITAL_DEPLOYMENT_V309={
  version:30.9,
  patch:'30.9.4-canonical-buy-bridge',
  minScore:68,
  canonicalProposalScore:SHADOW_LEARNING_V314.canonicalBuyScore,
  canonicalMinDataQuality:SHADOW_LEARNING_V314.canonicalMinDataQuality,
  canonicalMinOrthogonalConfirmations:SHADOW_LEARNING_V314.canonicalMinOrthogonalConfirmations,
  convictionScore:70,
  strongScore:75,
  veryStrongScore:80,
  exceptionalScore:85,
...
function normalizeScore(v){let x=num(v,NaN);if(!Number.isFinite(x)||x<=0)return null;if(x<=10)x*=10;return clamp(x,0,100)}
function score(c){for(const v of [c?.daytradeLiveScore,c?.decisionScore,c?.daytradeChanceScore,c?.pcDeepScore,c?.pcPreScore,c?.score]){const x=normalizeScore(v);if(x!==null)return x}return 0}
function proposalAssessment(c,universe){const legacy=score(c),canonical=canonicalEntryAssessmentV316(c,universe,[],.291);const canonicalEligible=canonical.score>=HIGH_SCORE_CAPITAL_DEPLOYMENT_V309.canonicalProposalScore&&canonical.dataQuality>=HIGH_SCORE_CAPITAL_DEPLOYMENT_V309.canonicalMinDataQuality&&canonical.orthogonalConfirmations>=HIGH_SCORE_CAPITAL_DEPLOYMENT_V309.canonicalMinOrthogonalConfirmations;return{legacy,canonical,canonicalEligible,effective:canonicalEligible?Math.max(legacy,canonical.score):legacy,eligible:legacy>=HIGH_SCORE_CAPITAL_DEPLOYMENT_V309.minScore||canonicalEligible}}
...
function allocationFor(c,positionCount,scoreOverride=null){const s=scoreOverride==null?score(c):num(scoreOverride),a=m5(c),b=m20(c);let pct=s>=90?85:s>=85?68:s>=80?50:s>=75?36:s>=70?24:12;if(positionCount===0)pct+=6;else if(positionCount===1)pct+=3;if(a>=.35)pct+=6;if(b>=.65)pct+=6;if(a<0||b<0)pct-=3;return +clamp(pct,8,HIGH_SCORE_CAPITAL_DEPLOYMENT_V309.maxAutoAllocationPct).toFixed(1)}
function quality(c,effectiveScore=null){const s=effectiveScore==null?score(c):num(effectiveScore),a=m5(c),b=m20(c),q=num(c?.entryQualityScore,num(c?.timingQualityScore,50)),acc=num(c?.acceleration5Pct,num(c?.momentumAcceleration5,0));return s+clamp(a*2,-2,3)+clamp(b*1.2,-2,3)+clamp(acc*1.2,-1.5,2)+clamp((q-50)/15,-2,2)}
...
export function enforceHighScoreCapitalDeploymentV309(plan,state={},input=null,brokerRows=[]){
 ...
 const rows=merged.filter(c=>!held.has(key(c))&&brokerExact(c)).map(c=>{const i=idx.get(key(c)),a=i===undefined?{}:actions[i],proposal=proposalAssessment(c,merged);return{c,s:key(c),a,i,score:proposal.effective,legacyScore:proposal.legacy,canonical:proposal.canonical,canonicalEligible:proposal.canonicalEligible,eligible:proposal.eligible,m5:m5(c),m20:m20(c),q:quality(c,proposal.effective),hard:hard(a,c)}}).filter(x=>Number.isFinite(x.q)).sort((a,b)=>b.q-a.q);
 const strong=rows.filter(x=>x.score>=70),canonicalProposalCandidates=rows.filter(x=>x.canonicalEligible).length,blockedHard=rows.filter(x=>x.eligible&&x.hard).length,blockedMomentum=rows.filter(x=>x.eligible&&!x.hard&&(x.m5<cst.mildPullback5mFloor||x.m20<cst.mildPullback20mFloor)).length;
 let injected=0,canonicalInjected=0,upgraded=0,winnerTopups=0;let chosen=null,topup=null;
 if(positions.length<cst.maxOpenPositions){
   chosen=rows.find(x=>x.eligible&&!x.hard&&x.m5>=cst.mildPullback5mFloor&&x.m20>=cst.mildPullback20mFloor)||null;
   if(chosen){
     const act=String(chosen.a?.action||'HOLD').toUpperCase();
     if(act!=='BUY'){
       const pct=allocationFor(chosen.c,positions.length,chosen.score),viaCanonical=chosen.canonicalEligible&&chosen.legacyScore<cst.minScore,buy={...chosen.a,symbol:chosen.s,name:chosen.c?.name,action:'BUY',allocation_pct:pct,confidence:clamp(.72+(chosen.score-68)*.012,.70,.93),highScoreCapitalDeploymentV309:true,canonicalProposalBridgeV317:viaCanonical,reason:viaCanonical?`V31.7 CANONICAL-PROPOSAL: ${chosen.s} erreicht ${chosen.canonical.score.toFixed(1)}/100 bei Datenqualitaet ${chosen.canonical.dataQuality.toFixed(1)} und ${chosen.canonical.orthogonalConfirmations} unabhaengiger Bestaetigung(en). Der alte V30.9-Score (${chosen.legacyScore.toFixed(1)}) darf einen gueltigen Canonical-Kandidaten nicht mehr vor der finalen V31.7-Lern-/Safety-Pruefung verlieren. ${pct.toFixed(1)}% Starter; der nachgelagerte V31.7-Filter kann negative gelernte Edge, Probation und Safety weiterhin vollstaendig blockieren.`:`V30.9 CAPITAL-DEPLOYMENT: ${chosen.s} ${chosen.score.toFixed(1)}/100 ist exakt Trade-Republic-verifiziert. Ein milder Pullback ist kein pauschaler Kaufblock mehr (5m ${chosen.m5>=0?'+':''}${chosen.m5.toFixed(2)}%, 20m ${chosen.m20>=0?'+':''}${chosen.m20.toFixed(2)}%). ${pct.toFixed(1)}% dynamische Startgroesse; bis 100% sind ohne Hebel technisch moeglich, aber nur bei entsprechend hoher Ueberzeugung. Harte News-, Quote-, FX-, Markt- und Re-Entry-Sperren bleiben unangetastet.`};
       if(chosen.i===undefined){idx.set(chosen.s,actions.length);actions.push(buy)}else actions[chosen.i]=buy;injected++;if(viaCanonical)canonicalInjected++;
     }else if(num(chosen.a?.allocation_pct)<12&&chosen.score>=70){const pct=allocationFor(chosen.c,positions.length,chosen.score);actions[chosen.i]={...chosen.a,allocation_pct:Math.max(num(chosen.a?.allocation_pct),pct),highScoreCapitalDeploymentV309:true,reason:`${String(chosen.a?.reason||'BUY').slice(0,500)} · V30.9: starke ${chosen.score.toFixed(1)}/100-Chance wird nicht nur mit Mini-Starter gefahren; dynamisch auf ${pct.toFixed(1)}% angehoben.`};upgraded++}
   }
 ...
 plan.actions=actions;plan.summary=`${String(plan.summary||'').slice(0,105)} · V30.9.4 Capital: ${injected} BUY (${canonicalInjected} Canonical-Bridge) · ${upgraded} Starter vergroessert · ${winnerTopups} Gewinner-Aufstockung · ${blockedMomentum} Momentumblock · ${blockedHard} hart blockiert.`;
 return{plan,counters:{brokerRows:master.size,exactCandidates:rows.length,strong70Plus:strong.length,canonicalProposalCandidates,injected,canonicalInjected,upgraded,winnerTopups,blockedMomentum,blockedHard,chosen:chosen?{symbol:chosen.s,score:+chosen.score.toFixed(1),legacyScore:+chosen.legacyScore.toFixed(1),canonicalScore:+chosen.canonical.score.toFixed(1),canonicalDataQuality:+chosen.canonical.dataQuality.toFixed(1),canonicalOrthogonal:chosen.canonical.orthogonalConfirmations,m5:+chosen.m5.toFixed(2),m20:+chosen.m20.toFixed(2)}:null,topup:topup?{symbol:topup.s,score:+topup.stable.toFixed(1),raw:+topup.raw.toFixed(1),pnlPct:+topup.pl.toFixed(2)}:null}};
}
...
export class HighScoreCapitalDeploymentV309{
 ...
 status(){return{enabled:true,...HIGH_SCORE_CAPITAL_DEPLOYMENT_V309,mode:'canonical-proposal-bridge+high-score-capital-deployment',mildPullbackCanBuy:true,hardSafetyNeverOverridden:true,noMiniStarterFor70Plus:true,winnerTopupWhenFourPositions:true,heldMasterEnrichment:true,noFixedAutoSinglePositionCap:true,brokerResolveCount:this.brokerResolveCount,latest:this.latest?.counters||null,rule:'Leeres/unterinvestiertes Depot darf einen gueltigen V31.7-Canonical-Kandidaten nicht mehr nur deshalb verlieren, weil der alte V30.9-Legacy-Score unter 68 liegt. Canonical BUY-Vorschlaege brauchen weiterhin exakte Trade-Republic-Verifizierung, mindestens 60 Score, 55 Datenqualitaet und eine unabhaengige Volumen- oder News-Bestaetigung; die nachgelagerte V31.7-Probation/Lern-/Safety-Pruefung bleibt finale Autoritaet. Alte 68+ Setups bleiben rueckwaertskompatibel kaufbar.'}}
}
