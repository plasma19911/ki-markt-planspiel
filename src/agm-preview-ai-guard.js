...
export class AgmPreviewAiGuard{
 ...
 async run(input){
  ...
  const calendar=await evaluateAgmCalendar(this.env,state,candidates,Date.now());this.latest=calendar;
  ...
  for(const ev of eligible){
   const symbol=key(ev),c=candidateMap.get(symbol),existing=actionMap.get(symbol);if(!c||held.has(symbol)||candidateHardBlock(c)||residual<2)continue;
   ...
   const reason=`FINAL-CONTROLLER V27.6 BUY AGM_PREVIEW: Hauptversammlung ${ev.daysUntil===1?'morgen':`in ${ev.daysUntil} Tagen`} · Vorab-Score ${Math.round(num(ev.score))}/100 (${ev.label}) · positive Gewinn-/Ausblickseinschätzung vor Termin bestätigt · technische und News-Bedingungen aktuell ausreichend · HV allein ist kein Kaufgrund · max. ${AGM_PREVIEW_RULES.maximumAllocationPct}% Cash vor den normalen Risiko-/Kosten-Gates.`;
  ...
  // Re-apply the canonical portfolio caps to ALL planned BUYs after adding AGM_PREVIEW.
  ...
  const finalActions=actions.map(a=>{
   ...
   if(!cap)return{symbol:key(a),action:'HOLD',confidence:Math.max(.68,num(a?.confidence,.68)),allocation_pct:0,reason:`PORTFOLIO-RISK V27.6: BUY nach finaler Depotrisikoprüfung verworfen. ${a.reason}`};
  ...
  plan.actions=finalActions;plan.summary=`${String(plan.summary||'FINAL-CONTROLLER').replace(/V27\.5/g,'V27.6')} · AGM-PREVIEW V27.6: ${eligible.length} positiv bewertete HV-Kandidat(en), ${added} zusätzlicher Vorab-BUY zur normalen finalen Prüfung weitergereicht.`;
