...
function watchFrom(entries,meta={}){
 ...
 return{version:3,updatedAt:new Date().toISOString(),candidateCount:candidates.length,target:WIDE_SWEEP_TARGET,mode:'WINDOWS_PC_WIDE_SWEEP',candidates,masterCount:num(meta?.masterCount),scannedCount:num(meta?.scannedCount),cycleMinutes:num(meta?.cycleMinutes),batchCount:num(meta?.batchCount),coreCount:num(meta?.coreCount),coreCycleMinutes:num(meta?.coreCycleMinutes),tailCount:num(meta?.tailCount),tailCycleMinutes:num(meta?.tailCycleMinutes),fullMasterCycleMinutes:num(meta?.fullMasterCycleMinutes,Math.max(num(meta?.coreCycleMinutes),num(meta?.tailCycleMinutes))),maxCoverageMinutesTarget:num(meta?.maxCoverageMinutesTarget),tailBatchesPerMinute:num(meta?.tailBatchesPerMinute),hotEveryMinutes:num(meta?.hotEveryMinutes,1),warmEveryMinutes:num(meta?.warmEveryMinutes,1),quietCoreEveryMinutes:num(meta?.quietCoreEveryMinutes,meta?.coreCycleMinutes),sourceBackoff:Boolean(meta?.sourceBackoff),pausedUntil:meta?.pausedUntil||null,lastError:meta?.lastError||null,confirmationRequired:true,forcedBuy:false,notice:'PC-Volluniversum-Sweep ist nur Discovery. Auffaellige Werte muessen danach die komplette Live-/Safety-/News-/Anti-Chase-/Venue-Pruefung bestehen.'};
}
