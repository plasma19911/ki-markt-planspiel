...
const fresh=(ts,ttl=WIDE_TTL_MS)=>{const t=Date.parse(String(ts||''));return Number.isFinite(t)&&Date.now()-t>=0&&Date.now()-t<ttl};
const blockedVenueSymbol=s=>/\.(?:V|NE|PK|OB)$/i.test(key(s));
...
export function normalizeWideSweepEntries(input,now=Date.now()){
 ...
 for(const x of arr(input).slice(0,240)){
  const symbol=key(x?.symbol);if(!symbol||symbol.length>28||blockedVenueSymbol(symbol))continue;
  const observed=Date.parse(String(x?.observedAt||x?.ts||''));if(!Number.isFinite(observed)||observed>now+60_000||now-observed>WIDE_TTL_MS)continue;
 ...
 constructor(ctx,env){
  ...
  if(assets?.fetch&&!assets.__pcWideSweepOverlay){
   ...
   assets.fetch=async(request,init)=>{
    ...
    const watch=this._readWideSweep();if(!watch||!fresh(watch.updatedAt)||!arr(watch.candidates).length)return Response.json(data,{headers:{'cache-control':'no-store'}});
    ...
    for(const c of arr(watch.candidates)){
     ...
     const row=resolve(c.symbol,index),s=key(row?.symbol);if(!row||!s||seen.has(s)||blockedVenueSymbol(s)||!liquidEnough(row))continue;
