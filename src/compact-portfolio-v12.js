import {MarketPortfolio as BasePortfolio} from './compact-portfolio-v11-base.js';
...
const SOURCES=[
  {name:'TradingView DE Biggest Losers',kind:'tv-de',market:'DE',url:'https://www.tradingview.com/markets/stocks-germany/market-movers-losers/'},
  {name:'Yahoo Day Losers',kind:'yahoo',market:'GLOBAL',url:'https://finance.yahoo.com/research-hub/screener/day_losers/'}
];
...
const fresh=(ts,ttl=REBOUND_TTL_MS)=>{const t=Date.parse(String(ts||''));return Number.isFinite(t)&&Date.now()-t>=0&&Date.now()-t<ttl};
...
function buildWatch(entries,rows,mode='PC_AGENT'){
  ...
  for(const e of entries){
    ...
    old.score+=Math.max(.15,5-(e.rank-1)*.13);old.bestRank=Math.min(old.bestRank,e.rank);old.sources.add(e.source);if(e.market==='DE')old.market='DE';scores.set(s,old);
  }
...
export class MarketPortfolio extends BasePortfolio{
  constructor(ctx,env){
    ...
    if(assets?.fetch&&!assets.__reboundUniverseOverlay){
      assets.__reboundUniverseOverlay=true;
      ...
      assets.fetch=async(request,init)=>{
        ...
        let watch=this._readReboundWatch();
        if(!watch||!fresh(watch.updatedAt))watch=await this._refreshReboundFallback(rows);
        const seen=new Set(arr(data?.equities).map(x=>key(x?.symbol))),extras=[];
        for(const c of arr(watch?.candidates)){
          if(extras.length>=REBOUND_TARGET)break;
          const row=resolve({symbol:c.symbol,market:c.market},index),s=key(row?.symbol);if(!row||!s||seen.has(s))continue;
          seen.add(s);extras.push({...row,reboundWatch:true,reboundRank:num(c.rank),reboundSource:c.source||watch?.mode||'Rebound Radar'});
        }
        return Response.json({...data,equities:[...arr(data?.equities),...extras],rebound_watch_count:num(watch?.candidateCount),rebound_scan_count:extras.length,rebound_scan_target:REBOUND_TARGET,scanner_slice_equity_count:arr(data?.equities).length+extras.length,scanner_mode:`${data?.scanner_mode||'LEADERS'}+REBOUND`},{headers:{'cache-control':'no-store'}});
      };
