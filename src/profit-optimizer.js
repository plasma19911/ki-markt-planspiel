...
function entryTiming(c){
 ...
 // Der Fast-Layer hat bereits Volumen-/MTF-Bestaetigungen. Falls volumeRatio im kompakten
 // Kandidatenprompt fehlt, darf ein starker BREAKOUT nicht faelschlich immer blockiert werden.
 const volumeOk=vol>=1.35||!Number.isFinite(Number(c?.volumeRatio));
...
function enrichInput(input,state){
 ...
 const policy='PROFIT-OPTIMIZER: Ziel ist maximaler erwarteter PAPER-TRADING-Gewinn nach realistischen Kosten, nicht maximale Handelsfrequenz. Konzentriere Kapital in wenige mehrfach bestaetigte Setups. Ein Forward-Watch-Kandidat darf frueh priorisiert werden, wenn Katalysator+Live-Reaktion+Liquiditaet zusammenpassen. Bei forwardDirectionUnknown=true niemals vor dem Ereignis die Richtung raten; erst nach positiver Live-Bestaetigung kaufen. Schwache Kandidaten duerfen 0% bekommen; Cash ist erlaubt, wenn kein positiver Erwartungswert vorliegt. Gewinner laufen lassen, aber bei klarer Reversal-/Peak-Giveback-Bestaetigung schneller umschichten. Kandidaten mit Infinity/NaN/kaputten Kursindikatoren niemals kaufen. ANTI-CHASE: Direkt am kurzfristigen Hoch nur bei echtem Breakout mit Volumen, positiver 5m/20m-Dynamik und nicht ueberhitztem RSI kaufen. Sonst auf Ruecksetzer/Re-Test warten; bestaetigte Pullback-/Re-Test-Setups bevorzugen. ENTRY-LEARNING: Ergebnisse 15/30/60 Minuten nach jedem Einstieg duerfen Timing-Score, Konfidenz und Positionsgroesse automatisch kalibrieren und wiederholt schlechte Chase-/Extended-Muster blockieren. ';
...
function scoreCandidate(c,fw,storage){
 ...
 if(day>5)s-=.45;if(day>8)s-=1.1;if(badEvidence(c))s-=20;if(timingLearn.block)s-=50;return s
}
...
function optimizeResponse(r,input,state,storage){
 ...
 const buyMap=new Map();for(const a of arr(j.actions)){if(String(a?.action||'').toUpperCase()!=='BUY')continue;const c=cMap.get(key(a)),learn=c?getEntryTimingAdjustment(storage,c):null;if(!c||heldSet.has(key(a))||badEvidence(c)||chaseBlocked(c)||learn?.block||timingBlocked.has(key(a)))continue;buyMap.set(key(a),{...a,_score:scoreCandidate(c,fw.get(key(a)),storage),_cand:c,_timingLearn:learn})}
 if(!buyMap.size){for(const c of candidates){const learn=getEntryTimingAdjustment(storage,c);if(heldSet.has(key(c))||badEvidence(c)||chaseBlocked(c)||learn.block||timingBlocked.has(key(c)))continue;if(String(c?.eventRisk||'').toUpperCase()==='HIGH'||String(c?.momentumSellSignal||'').toUpperCase()==='STRONG')continue;const sc=scoreCandidate(c,fw.get(key(c)),storage);if(sc>=7.2){const timing=entryTiming(c);buyMap.set(key(c),{symbol:key(c),action:'BUY',confidence:clamp(.52+sc*.035+num(learn.confidenceDelta),.58,.9),allocation_pct:1,reason:`PROFIT-OPTIMIZER: Erwartungswert ${sc.toFixed(2)} · ${timing.pullbackRetest?'Ruecksetzer/Re-Test bestaetigt':'Live+Katalysator bestaetigt'}${learn.reason?` · ${learn.reason}`:''}`,_score:sc,_cand:c,_timingLearn:learn});break}}
 ...
 if(deploy>0&&ranked.length){
   ...
   buys=chosen.map((x,i)=>{const timing=entryTiming(x._cand),learn=x._timingLearn||getEntryTimingAdjustment(storage,x._cand),learnCap=Math.max(12,baseCap*num(learn.sizeMultiplier,1)),cap=timing.nearHigh?Math.min(learnCap,35):learnCap;let pct=i===chosen.length-1?Math.min(remaining,cap):Math.min(cap,deploy*raw[i]/sum);pct=Math.max(0,Math.min(remaining,pct));remaining-=pct;const timingText=timing.nearHigh?'Breakout-Einstieg, Groesse begrenzt':timing.pullbackRetest?'Ruecksetzer/Re-Test':'Timing neutral',learnText=learn.reason?` · ${learn.reason}`:'';return{symbol:key(x),action:'BUY',confidence:clamp(num(x.confidence,.6)+num(learn.confidenceDelta),.5,.95),allocation_pct:+pct.toFixed(2),reason:`${String(x.reason||'').slice(0,190)} · PROFIT-SCORE ${x._score.toFixed(2)} · ${timingText}${learnText} · Kapital ${pct.toFixed(1)}%`}}).filter(x=>x.allocation_pct>0.01);
 ...
 j.actions=[...sells,...buys,...keptHolds];j.summary=`${String(j.summary||'').slice(0,210)} · PROFIT-OPTIMIZER+ENTRY-LEARNING: bester Score ${Number.isFinite(best)?best.toFixed(2):'–'}, ${buys.reduce((a,x)=>a+num(x.allocation_pct),0).toFixed(0)}% Cash fuer ${buys.length} BUY(s); 15/30/60m-Timinglernen aktiv.`;
...
export class ProfitOptimizerAiGuard{
 ...
 async run(model,input){const joined=String(arr(input?.messages).map(x=>x?.content||'').join('\n')),isPlan=joined.includes('Kandidaten=')&&joined.includes('JSON-only');if(!isPlan)return this.base.run(model,input);const state=this.adapter?.peekState?.()||null,next=enrichInput(input,state);const r=await this.base.run(model,next);return optimizeResponse(r,next,state,this.storage)}
}
