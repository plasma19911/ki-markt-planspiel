...
export function rotationCostDecision({held={},action={},storage=null}={}){
 const replay=getReplayRotationAdjustment(storage),age=ageMin(held),notional=Math.max(0,num(held?.invested,held?.amount)),gap=reasonGap(action?.reason),smallPenalty=notional>0&&notional<250?.55:notional>0&&notional<500?.35:.10,minAge=10+num(replay.minAgeBonusMinutes),minGap=.80+smallPenalty+num(replay.gapBonus),hard=hardExit(held);
 const allow=hard||(age>=minAge&&(gap==null||gap>=minGap));
 return{allow,age:+age.toFixed(1),gap,minAge,minGap:+minGap.toFixed(2),notional:+notional.toFixed(2),hardExit:hard,replay,reason:allow?(hard?'harter Exit':'Rotation nach Kosten-Hysterese erlaubt'):`Rotation gebremst: ${age.toFixed(1)} Min. < ${minAge} Min. oder Score-Abstand ${gap==null?'–':gap.toFixed(2)} < ${minGap.toFixed(2)}; kleine Orders/Gebuehren beruecksichtigt`}
}
