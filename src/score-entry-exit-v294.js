...
function defaults(){return{version:29.4,entries:{},recent:[],stats:{entryScoresStored:0,recoveredEntryScores:0,positiveScoreExits:0,negativeScoreExits:0,partialScoreWaits:0,partialScoreFreezes:0,flatChartCaps:0,positiveScoreWaitsForChart:0},updatedAt:null}}
...
function recoverEntry(s,p,row,candidate,mem,v296,v293,now){
  ...
  const recovered=recoverScoreSeed(s,v296,v293),rowScore=firstFinite(row?.decisionScore,row?.holdScore,row?.buyScore),full=!row?.partial&&finite(rowScore),score=recovered?num(recovered.stable):full?rowScore:NaN;
  if(!finite(score))return null;
  const currentPrice=priceOf(p,candidate,row),entryPrice=entryPriceOf(p,candidate,currentPrice),source=recovered?'RECOVERED_COHERENT_SCORE':'FIRST_V294_FULL_SCORE';
  const e={score:+score.toFixed(1),lastStable:+score.toFixed(1),entryPrice,lastPrice:currentPrice||entryPrice,at:now,lastAt:now,source,fullSeen:Boolean(full)};mem.entries[s]=e;return e;
}
...
export function positionScoresV294(state={},storage=null,now=Date.now(),update=false,workingMem=null){
  ...
  for(const p of arr(state?.positions)){
    ...
    const hadEntry=Boolean(mem.entries[s]),entry=recoverEntry(s,p,row,candidate,mem,v296,v293,now);if(!entry)continue;if(!hadEntry&&String(entry.source).startsWith('RECOVERED'))mem.stats.recoveredEntryScores++;
...
export function enforceScoreEntryExitV294(plan,state={},storage=null,now=Date.now()){
  ...
  const base=stableDecisionScoresV296(state,storage,now,false),candidateScore=new Map(arr(base.ranking).map(r=>[key(r),r])),held=new Set(arr(state?.positions).map(p=>key(p)).filter(Boolean)),actions=plan.actions.map(a=>({...a})),actionIndex=new Map();actions.forEach((a,i)=>{const s=key(a);if(s&&!actionIndex.has(s))actionIndex.set(s,i)});
  ...
  for(const a of actions){
    const s=key(a);if(!s||held.has(s)||String(a?.action||'').toUpperCase()!=='BUY')continue;const row=candidateScore.get(s);if(!row)continue;const score=firstFinite(row?.decisionScore,row?.buyScore);if(!finite(score))continue;const c=arr(state?.candidates).find(x=>key(x)===s)||{},price=priceOf({},c,row);mem.entries[s]={score:+score.toFixed(1),lastStable:+score.toFixed(1),entryPrice:price,lastPrice:price,at:now,lastAt:now,source:'BUY_DECISION_SCORE_V296',fullSeen:true};counters.entryScoresStored++;
  }
  ...
  for(const [s,e] of Object.entries(mem.entries))if(!held.has(s)&&!actions.some(a=>key(a)===s&&String(a?.action||'').toUpperCase()==='BUY')&&now-num(e?.at,0)>24*3600_000)delete mem.entries[s];
  for(const k of Object.keys(counters))mem.stats[k]=num(mem.stats[k])+num(counters[k]);mem.updatedAt=new Date(now).toISOString();mem.recent.push({at:now,...counters});mem.recent=mem.recent.slice(-120);write(storage,mem);plan.actions=actions;plan.summary=`${String(plan.summary||'').slice(0,145)} · V29.4/V29.6: +10 bei positivem Chart => SELL ${counters.positiveScoreExits} · -15 => SELL ${counters.negativeScoreExits}.`;
...
export class ScoreEntryExitGuardV294{
  ...
  status(){const state=typeof this.getState==='function'?(this.getState()||{}):{},mem={...defaults(),...read(this.storage,defaults())},scores=positionScoresV294(state,this.storage,typeof this.now==='function'?this.now():Date.now(),false);return{enabled:true,version:29.4,scoreCoreVersion:29.6,authoritativePositionScoreExit:true,chartAnchoredPositionScore:true,positiveExitRequiresPositiveChart:true,thresholds:SCORE_ENTRY_EXIT_V294,entries:mem.entries||{},positionScores:scores.positionScores,latest:this.latest?.counters||null,stats:mem.stats||{},rule:'Kaufscore bleibt feste Basis. Teilwerte duerfen die Skala nicht wechseln. +10 fuehrt nur bei positivem Chart seit Kauf zum SELL; -15 fuehrt zum SELL. Die Scores stammen aus dem zeit-/qualitaetsstabilisierten V29.6-Core.'}}
}
