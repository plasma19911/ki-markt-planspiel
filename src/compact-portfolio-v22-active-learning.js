...
// PAPER-TRADING ONLY. V27.3 keeps V27.2 forward learning and adds deterministic
// net-fee-aware loss-sell and order-economics invariants after the final controller.
export class MarketPortfolio extends BasePortfolio{
 constructor(ctx,env){
  ...
  if(ai?.run&&!ai.__finalDecisionControllerV273){
   ...
   wrapped.__finalDecisionControllerV273=true;
   wrapped.__lossSellInvariantV273=true;
 ...
 async status(){
  ...
  s.finalDecisionPolicy={
   enabled:true,version:27.3,paperTradingOnly:true,mode:'SINGLE_AUTHORITATIVE_FINAL_CONTROLLER_PLUS_DETERMINISTIC_TRADE_INVARIANTS',oneFinalActionPerSymbol:true,
   entryTypes:['EARLY_BREAKOUT','PULLBACK_RECLAIM','BASE_RECLAIM'],dynamicCapitalDeployment:'Qualitäts- und Chancenbreiten-basiert, anschließend durch Depotrisiko, Vorwärtsprognose und Orderökonomie begrenzt',
   ...
   regressionTests:'tests/final-decision-controller-v26.test.mjs + tests/run-reset-hygiene-v262.test.mjs + tests/v27-risk-learning.test.mjs + tests/v271-anti-flipflop.test.mjs + tests/v272-forward-curve.test.mjs + tests/v273-loss-sell-invariant.test.mjs',
   rule:'V27.3 verhindert SAGILITY-/GUBRA-artige widersprüchliche Verlustverkäufe und den PARAS-Fehler, bei dem ein Brutto-Kursgewinn als PROFIT EXIT behandelt wurde, obwohl der echte ZERO-Netto-Trade nach Kauf-/Verkaufsgebühren negativ war. Netto-P/L wird vor SELL mit ZERO-Gebühren und Slippage geschätzt. Ein PROFIT EXIT darf keinen Nettoverlust schließen. Neue Mini-BUYs werden blockiert, wenn erwartete Roundtrip-Brokerkosten plus Slippage mehr als 1,5% des handelbaren Notionals kosten. Echte Hard-Risiken und bestätigte tiefe Strukturbrüche bleiben verkaufbar.'
  };
  s.portfolioRiskPolicy={enabled:true,version:27.3,method:'FACTOR_CONCENTRATION_PROXY',maxSinglePositionPct:V27_RISK_LIMITS.maxSinglePositionPct,maxThemePct:V27_RISK_LIMITS.maxThemePct,maxRegionPct:V27_RISK_LIMITS.maxRegionPct,maxForeignCurrencyPct:V27_RISK_LIMITS.maxForeignCurrencyPct,maxBaseCurrencyPct:V27_RISK_LIMITS.maxBaseCurrencyPct,note:'Proxy-Korrelation über Thema, Region und Währung; keine behauptete exakte Kovarianzmatrix. Gleichzeitige BUYs werden gemeinsam gegen dieselben Limits gerechnet.'};
  s.learningCalibrationPolicy={enabled:true,version:27.3,method:'SHRUNK_EMPIRICAL_SETUP_EXPECTATION_PLUS_FORWARD_CURVE',timingHorizonsMinutes:learning?.horizonsMinutes||[15,30,60],matureTimingBuckets:learning?.matureTimingBuckets||0,activeCleanTimedCheckpoints:learning?.timedCheckpoints||0,priorVersion:FAST_CALIBRATION?.version||null,priorHoldoutBuySamples:FAST_CALIBRATION?.validation?.holdoutBuySamples||0,priorBuyHitRate:FAST_CALIBRATION?.validation?.holdoutBuyHitRate??null,priorBuyMeanPct:FAST_CALIBRATION?.validation?.holdoutBuyMeanPct??null,forwardCurve:forward,confidenceIsCalibratedProbability:false,note:'Historischer Holdout und saubere Live-Buckets bleiben die Basis. Die 5/15/30-Minuten-Prognose wird konservativ hinzugemischt und lernt auch aus nicht gekauften Kandidaten. Interne Konfidenz bleibt keine garantierte Erfolgswahrscheinlichkeit.'};
  s.forwardCurveLearningPolicy={...forward,enabled:true,version:27.3,learnsFromUnboughtCandidates:true,usesMarketBreadthRegime:true,probabilisticOnly:true,rule:'Beobachtung -> tatsächlicher Kurs nach 5/15/30 Min. -> empirische Treffer-/Erwartungswerte für ähnliche Kurven und Marktregime. Keine Vorhersage wird als sicher behandelt.'};
  s.learningQuarantinePolicy={enabled:true,version:27.3,...quarantine,keepsOrdinaryLosingTrades:true,legacyAggregatesArchivedNotActive:true,contradictoryExitHoldHardExitQuarantined:true,immediateReentryAfterBugSellQuarantined:true,rule:'Widersprüchliche HARD-EXIT/EXIT-HOLD-Fälle und direkte Reentries danach werden aus aktiven Lernproben entfernt. Normale schlechte Trades bleiben Lernmaterial.'};
  s.runIsolationPolicy={enabled:true,version:27.3,newRunGetsFreshTradingState:true,clearsRunScopedDecisionState:true,longTermLearningPreserved:true,legacyPreV27LearningPreservedAsArchive:true,forwardCurveLearningPreserved:true,dailyAiBudgetPreserved:true,runtimeTradeConfigPreserved:true,automaticScaleUp:false,rule:'Neu starten trennt laufbezogenen Handelszustand. Saubere langfristige Lernmodelle inklusive Vorwärtskurven-Lernen bleiben erhalten.'};
  if(s.executionModel)s.executionModel={...s.executionModel,finalDecisionControllerV273:true,oneFinalActionPerSymbol:true,automaticScaleUp:false,automaticRepeatScaleUpBlocked:true,residualCashOrderBlocked:true,restartIsolation:true,timeBasedLossExit:false,lossExitNeedsIndependentThesisInvalidation:true,portfolioRiskCaps:true,antiFlipFlop:true,forwardCurveForecasting:true,lossSellInvariant:true,orderEconomicsInvariant:true};
  if(s.profitOptimizer)s.profitOptimizer={...s.profitOptimizer,finalDecisionControllerV273:true,automaticScaleUp:false,entryTimingFirst:true,hardInnerSafetyHoldPreserved:true,freshExitContextMerged:true,restartIsolation:true,timeBasedLossExit:false,calibratedSetupExpectation:true,antiFlipFlop:true,forwardCurveForecasting:true,lossSellInvariant:true,orderEconomicsInvariant:true};
  return s;
 }
}
