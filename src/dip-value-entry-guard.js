...
function metrics(c={}){
 ...
 const earlyFallingQuality=(wideDipDiscovery||foresight)&&((score>=2.15&&confidence>=.58)||(score>=1.90&&confidence>=.64)||(score>=1.80&&confidence>=.60&&news>=.12));
 const quality=regularQuality||dipQuality;
 return{score,confidence,news,day,m5,m20,accel,rsi,drawKnown,draw,event,state,sell,hardSafe,quality,regularQuality,dipQuality,earlyFallingQuality,wideDipDiscovery,foresight};
}
...
export function assessDipValueEntry(c={}){
 ...
 // PRE-DIP: Noch leicht fallend ist erlaubt, wenn Breitscan/Foresight bereits einen
 // kontrollierten Dip sieht UND die Beschleunigung klar nach oben dreht. Nur kleine
 // Starterposition; der groessere Ausbau bleibt dem bestaetigten Rebound vorbehalten.
 const earlyFallingStarter=x.hardSafe&&x.earlyFallingQuality&&inDip&&falling&&x.m5>=-.32&&x.m5<=.04&&x.m20>=-1.35&&x.m20<=.12&&x.accel>=.025&&x.rsi>=29&&x.rsi<=60&&x.day>=-7&&x.day<=.60;
 ...
 const mode=rebound?'DIP_REBOUND':earlyFallingStarter?'EARLY_FALLING_STARTER':dipStarter?(deepDip?'DEEP_DIP_STARTER':'DIP_STARTER'):unknownValue?'VALUE_STARTER':exceptionalBreakout?'EXCEPTIONAL_BREAKOUT':'WAIT_FOR_VALUE';
 const allow=rebound||earlyFallingStarter||dipStarter||unknownValue||exceptionalBreakout;
 let cap=rebound?30:earlyFallingStarter?(deepDip?6:9):dipStarter?(deepDip?16:20):unknownValue?14:exceptionalBreakout?5:0;
 if(x.foresight)cap=Math.min(cap,rebound?18:earlyFallingStarter?8:12);
 const dipBonus=x.wideDipDiscovery?1.65:(x.day<0?Math.min(1.1,Math.abs(x.day)*.20):0);
 const valueScore=x.score*1.10+x.confidence*2+x.news*.20+dipBonus+(inDip?Math.min(3.5,Math.abs(x.draw))*.55:0)+(x.accel>0?Math.min(.35,x.accel)*2:0)-(nearHigh?2.35:0)-(x.day>0?Math.min(6,x.day)*.48:0)+(x.foresight?0.55:0)+(earlyFallingStarter?.45:0);
 const blockers=[];
 if(!x.hardSafe)blockers.push('harte Safety');
 if(!x.quality&&!x.earlyFallingQuality)blockers.push('Qualitaet/Konfidenz');
 if(nearHigh&&!exceptionalBreakout)blockers.push('zu nah am 20m-Hoch');
 if(inDip&&!fallSlowing&&!rebound&&!earlyFallingStarter)blockers.push('Abwaertsdruck noch nicht gebremst');
 ...
 return{allow,mode,cap,valueScore:+valueScore.toFixed(3),...x,falling,fallSlowing,inDip,deepDip,rebound,earlyFallingStarter,dipStarter,unknownValue,exceptionalBreakout,blockers:[...new Set(blockers)]};
}
...
function reasonFor(q){
 ...
 if(q.mode==='EARLY_FALLING_STARTER')return `${src}frueher Starter im noch leicht fallenden Kurs: Ruecksetzer ${q.draw?.toFixed(2)}%, 5m ${q.m5.toFixed(2)}%, Verkaufsdruck bremst deutlich (Beschl. +${q.accel.toFixed(2)}); erst bei Bestaetigung groesser`;
...
function postProcess(r,input){
 ...
 if(blocked.length||hasBuy){const entryBuys=finalActions.filter(a=>String(a?.action||'').toUpperCase()==='BUY'),dipBuys=entryBuys.filter(a=>/DIP|VALUE|FALLING/i.test(String(a?.reason||''))).length;plan.summary=`${String(plan.summary||'').slice(0,130)} · EARLY-DIP: ${blocked.length} ungeeignete Idee(n) gestoppt; ${entryBuys.length?`${dipBuys||entryBuys.length} Einstieg(e), fruehe Ruecksetzer priorisiert`:'kein Kauf – aktuell kein kontrollierter Einstieg'}.`}
