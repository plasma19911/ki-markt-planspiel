...
export class MarketPortfolio extends BasePortfolio{
  ...
  async _buildStatus(includeAudit=true){
    ...
    if(includeAudit)s.decisionAuditRecent=await this.decisionAudit(20);else{s.decisionAuditRecent=[];s.decisionAuditWindow=0;if(Array.isArray(s.history))s.history=s.history.slice(0,60);if(Array.isArray(s.aiLog))s.aiLog=s.aiLog.slice(0,40);if(Array.isArray(s.snapshots))s.snapshots=s.snapshots.slice(0,60)}
