...
export class MarketPortfolio extends BasePortfolio{
  ...
  async _buildStatus(includeAudit=true){
    ...
    const statusCandidates=arr(s.candidates),cost=estimatedRoundTripCostPctV314(s);s.candidates=statusCandidates.map(c=>{const a=canonicalEntryAssessmentV316(c,statusCandidates,arr(shadowMemory?.matured),cost);return{...c,legacy_score:c.score,legacy_decision_score:c.decisionScore??null,score:a.score,decisionScore:a.score,entryScoreV316:a.score,entryLabelV316:a.label,dataQualityV316:a.dataQuality,expectedNetEdgePctV316:a.expectedNetEdgePct,entryScoreSamplesV316:a.samples,entryScoreComponentsV316:a.components,scoreSource:'V31.6_CANONICAL_ENTRY_SCORE'}}).sort((a,b)=>Number(b.score)-Number(a.score));
    s.runtimeVersion='V31.6';s.liveDecisionVersion='V31.6';s.predictiveLearningVersion='V31.2';s.outcomeLearningVersion='V31.2';s.unifiedDecisionCorePolicy={...policy,brokerMaster:{...(this.__brokerRowsMeta||{})}};s.predictiveLearningPolicy={enabled:true,...learning,insideUnifiedAuthority:true};s.outcomeLearningPolicy={enabled:true,...learning,insideUnifiedAuthority:true};s.shadowLearningPolicy={enabled:true,...(policy.shadowLearning||{}),...persistedShadow,insideUnifiedAuthority:true};s.expectancyCorePolicy={enabled:true,version:31.6,mode:'inside-unified-capital-velocity+canonical-entry-score',calibration:persistedShadow.canonicalCalibration||null,...policy.expectancy};
    s.architecture={version:'31.6-canonical-entry-score+31.5-evidence-fusion+31.4-shadow-calibration',outerDecisionLayers:1,outerAuthority:'UnifiedDecisionCoreV310',internalDecisionPasses:['V31.2 continuous outcome learning/early entry','V30.9 legacy opportunity proposal','V31.6 one canonical score/data quality/net edge final entry approval','V31.3 capital-velocity exits/rotation/sizing'],removedOuterWrapperStack:['FinalSellAuthorityV308','HighScoreCapitalDeploymentV309','ExpectancyCoreV310-as-separate-wrapper'],legacyExecutionBase:'V30.7 manual controls/anti-churn and lower execution core',persistentDecisionAudit:true,auditStorageKey:UNIFIED_DECISION_CORE_V310.auditStorageKey,persistentPredictiveLearning:true,persistentOutcomeLearning:true,persistentShadowLearning:true,predictiveLearningStorageKey:OUTCOME_LEARNING_V312.storageKey,outcomeLearningStorageKey:OUTCOME_LEARNING_V312.storageKey,shadowLearningStorageKey:policy.shadowLearning?.storageKey||'shadow-learning-v314',stateMergeUsesRawCandidates:true,brokerMasterSource:this.__brokerRowsMeta?.source||'none'};
    ...
    s.canonicalScorePolicy={enabled:true,version:31.6,authoritative:true,canonicalScale:'0-100',oneFinalEntryScorePerSymbolPerScan:true,buyZoneMin:60,dataQualityMin:55,learningMode:canonicalCalibrated?'KALIBRIERT':'WARMUP',legacyScoresDiagnosticOnly:true,expectedNetEdgeAfterCosts:true,minimumSamplesPerFivePointBand:25,roundTripCostPct:cost,missingDataReducesQuality:true,components:['trend','relativeStrength','volume','news'],riskPenalties:['overbought','dayExtension','negativeAcceleration','negativeNews','HIGH_CHASE'],labels:{buy:'KAUFZONE',watch:'BEOBACHTEN',weak:'ZU SCHWACH'},sellLogicSeparate:true,continuousOutcomeFeedback:true,shadowCandidateCalibration:true};
