...
const NEWS_HEADERS={'accept':'application/rss+xml,application/xml,text/xml,*/*;q=0.7','user-agent':'Mozilla/5.0 (compatible; KI-Markt-Planspiel/CatalystGapV2.3)'};
...
function rssItems(text,source='Google Catalyst Gap',limit=18){const out=[];for(const m of String(text||'').matchAll(/<item>([\s\S]*?)<\/item>/gi)){const b=m[1],title=decode(b.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]),pub=decode(b.match(/<pubDate[^>]*>([\s\S]*?)<\/pubDate>/i)?.[1]);if(title)out.push({headline:title,title,publishedAt:pub&&Number.isFinite(Date.parse(pub))?new Date(pub).toISOString():null,source})}return out.slice(0,limit)}
...
function companySearchName(c={}){const words=companyWords(c),full=String(c?.name||key(c)).replace(/"/g,'').trim();if(!words.length)return full;const core=words.slice(0,Math.min(3,words.length)).join(' ');return core.length>=5?core:full}
...
function needsTargetedGapLookup(m,existing){
 if(existing?.impact>=4||m.event==='HIGH')return false;
 const day=Math.abs(m.day),m20=Math.abs(m.m20);
 // Der breite News-Radar bleibt immer aktiv. Dieser Zusatzrequest ist nur fuer echte
 // Preis/News-Diskrepanzen gedacht, nicht als zweite generische News-Abfrage pro Scan.
 return day>=4||m20>=2||(m.score>=4.8&&day>=2.5&&m.newsConfidence<.65);
}
async function discoverMissingCatalysts(candidates,state){
 const ranked=arr(candidates).map(c=>{const m=candidateMetrics(c),known=[...existingRowsFor(key(c),state,c),...cachedRows(c)],existing=strongestNewsImpact(known),gap=Math.abs(m.day)*(1.15-clamp(m.newsConfidence,0,1))*(1+Math.max(0,4-existing.impact)*.22);return{c,m,existing,gap}}).filter(x=>needsTargetedGapLookup(x.m,x.existing)).sort((a,b)=>b.gap-a.gap),target=ranked[0]?.c||null,out=new Map();
 // Maximal ein zusaetzlicher News-Request pro Entscheidung, aber nur wenn wirklich
 // eine auffaellige Preisbewegung ohne ausreichend starken bekannten Katalysator vorliegt.
 // Leere/fehlgeschlagene Checks werden 120 Sekunden gecacht.
