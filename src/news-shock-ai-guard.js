...
function postProcess(r,input,getState,storage){
 ...
 const profiles=candidates.filter(c=>!heldSet.has(key(c))).map(c=>({c,p:shockProfile(c,state,learn)}));
...
export class NewsShockAiGuard{
 ...
 async run(model,input){const r=await this.base.run(model,input);return postProcess(r,input,this.getState,this.storage)}
}
