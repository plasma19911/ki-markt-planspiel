...
async function recheck(info,prior,fxFallback){
 ...
 const e9=ema(cl,9),e21=ema(cl,21),rr=rsi(cl),m5=(price/cl.at(-6)-1)*100,m20=(price/cl.at(-21)-1)*100,prev5=(cl.at(-6)/cl.at(-11)-1)*100,accel=m5-prev5,high20=Math.max(...cl.slice(-21,-1)),draw=high20?(price/high20-1)*100:0,pclose=num(normalizeQuotePrice(res.meta?.previousClose,quoteUnit.rawCurrency),cl[0]),day=pclose?(price/pclose-1)*100:0,vb=vol.slice(-21,-1).filter(x=>x>0),va=vb.length?vb.reduce((a,b)=>a+b,0)/vb.length:0,lastVol=num(vol.at(-1)),previousVol=num(vol.at(-2)),vr=va>0&&lastVol>0?lastVol/va:va>0&&previousVol>0?previousVol/va:1;
