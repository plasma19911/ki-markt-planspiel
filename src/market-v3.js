...
async function recheck(info,prior,fxFallback){
 ...
 const res=got.x,{cl,vol,ts}=barsFrom(res);if(cl.length<22)return{candidate:null,error:'Zu wenig Minuten',sourceOk:false};
...
async function recheckForesight(info,fxFallback=1){
 const got=await chart(info.symbol);if(!got.x)return{candidate:null,error:got.error,sourceOk:false};const res=got.x,{cl,vol,ts}=barsFrom(res);if(cl.length<22)return{candidate:null,error:'Zu wenig Minuten',sourceOk:false};
 const price=cl.at(-1),last=ts.at(-1)||num(res.meta?.regularMarketTime);if(!(last>0&&(Date.now()/1000-last)<5*60))return{candidate:null,error:'1m-Kurs nicht frisch',sourceOk:false};
 const e9=ema(cl,9),e21=ema(cl,21),rr=rsi(cl),m5=(price/cl.at(-6)-1)*100,m20=(price/cl.at(-21)-1)*100,prev5=(cl.at(-6)/cl.at(-11)-1)*100,accel=m5-prev5,high20=Math.max(...cl.slice(-21,-1)),draw=high20?(price/high20-1)*100:0,pclose=num(res.meta?.previousClose,cl[0]),day=pclose?(price/pclose-1)*100:0,vb=vol.slice(-21,-1).filter(x=>x>0),va=vb.length?vb.reduce((a,b)=>a+b,0)/vb.length:0,vr=va?num(vol.at(-1))/va:1;
