...
// Mehr zweite Chancen und ein eigener Early-Dip-Pfad. Der normale Basisscanner
// bleibt fuer Momentum/News bestehen; kontrollierte Ruecksetzer bekommen zusaetzlich
// eine direkte 1m-Pruefung, damit sie nicht schon im Momentum-Vorfilter verschwinden.
const RECHECK_MAX=6;
...
async function recheck(info,prior,fxFallback){
 ...
 const technical=clamp(.28+clamp(Math.abs(score)/10,0,.35)+clamp((vr-1)/4,0,.12),0,1),priorConf=clamp(num(prior?.confidence),0,1),trendBonus=trendUp?.06:0,confidence=clamp(technical*.58+priorConf*.42+trendBonus,0,1),eventRisk=String(prior?.event_risk||'NONE').toUpperCase();if(score<3.15||confidence<.55||m20<=-.20||state==='REVERSAL'||eventRisk==='HIGH')return{candidate:null,error:`Zweitcheck verworfen: Score ${score.toFixed(2)}, Konfidenz ${Math.round(confidence*100)}%`};
...
async function recheckForesight(info,fxFallback=1){
 ...
 const confidence=clamp(.50+score*.042+(braking||turning?.055:0)+(info.pcWideSweep?.025:0),.50,.78),eligible=inValue&&dayOk&&rsiOk&&notCrash&&(braking||turning)&&score>=3.05&&confidence>=.58;
...
export async function scanMarket(env,cfg,heldSymbols=[]){
 ...
 candidates.sort((a,b)=>(num(b?.score)+(b?.foresightDip?.7:0))-(num(a?.score)+(a?.foresightDip?.7:0)));
