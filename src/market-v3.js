...
async function secondChanceDeep(info,prior,fxFallback=1){
  ...
  for(let i=0;i<rawClose.length;i++){const c=Number(rawClose[i]);if(!Number.isFinite(c)||c<=0)continue;cl.push(c);vol.push(Math.max(0,num(rawVol[i])));validTimes.push(num(times[i]))}
  ...
  const technicalConfidence=clamp(.28+clamp(Math.abs(score)/10,0,.35)+clamp((vr-1)/4,0,.12),0,1),priorConf=clamp(num(prior?.confidence),0,1),trendBonus=trendUp?.06:0,confidence=clamp(technicalConfidence*.58+priorConf*.42+trendBonus,0,1),eventRisk=String(prior?.event_risk||'NONE').toUpperCase();
