...
function defaults(){return{version:29.6,candidates:{},positions:{},stats:{updates:0,largeRawJumps:0,qualityAdjusted:0,timeSmoothed:0,chartAccelerated:0,forcedBuys:0,blockedBelow56:0,reentryBlocks:0,reentryUnlocks:0},updatedAt:null}}
...
export function enforceDecisionScoreV296(plan,state={},storage=null,now=Date.now()){
  ...
  re.locks={...(re.locks||{})};re.stats={...reentryDefaults().stats,...(re.stats||{})};const actions=plan.actions.map(a=>({...a})),idx=new Map();actions.forEach((a,i)=>{const s=key(a);if(s&&!idx.has(s))idx.set(s,i)});const counters={forcedBuys:0,blockedBelow56:0,reentryBlocks:0,reentryUnlocks:0,reentryLocksCreated:0};

  for(const a of actions){
    const s=key(a);if(!s||!positions.has(s)||String(a?.action||'').toUpperCase()!=='SELL'||a?.scoreExitV294!==true)continue;const delta=num(a?.scoreExitDelta,0),kind=String(a?.scoreExitKind||''),exitScore=firstFinite(a?.scoreExitCurrent,a?.scoreExitEntry);
  ...
  for(const [s,lock] of Object.entries(re.locks)){
    if(positions.has(s))continue;const row=by.get(s);if(!row)continue;const score=row.decisionScore,ready=lock.kind==='PLUS_10'?score<=num(lock.rearmScore,56):score>=num(lock.rearmScore,56);
  ...
  for(const row of scored.ranking){
    ...
    if(score>=DECISION_SCORE_V296.immediateBuyMin&&!locked){const tier=tierFor(score),cash=Math.max(0,firstFinite(state?.config?.cash,state?.cash,0)),pct=entryAllocationPctV290(cash,{score,tier}),c=candidateMap.get(s)||{},next={...(existing!==undefined?actions[existing]:{}),symbol:s,name:c?.name||undefined,action:'BUY',allocation_pct:pct,confidence:clamp(.60+(score-56)*.006,.60,.90),reason:`V29.6 SOFORT-BUY: DecisionScore ${score.toFixed(1)}/100 >= 56. Zeit-/Qualitäts-geglättet, bei echter Chartbewegung schneller reaktiv.`};if(existing!==undefined)actions[existing]=next;else{idx.set(s,actions.length);actions.push(next)}counters.forcedBuys++}
      ...
      const why=locked?(lock.kind==='PLUS_10'?`Profit-Exit bei ${num(lock.exitScore).toFixed(1)}: erst nach Score-Rücksetzer bis ${num(lock.rearmScore).toFixed(1)} erneut einsteigen.`:`Schwäche-Exit bei ${num(lock.exitScore).toFixed(1)}: erst nach Score-Erholung bis ${num(lock.rearmScore).toFixed(1)} erneut einsteigen.`):`DecisionScore ${score.toFixed(1)}/100 < 56.`;actions[existing]={...actions[existing],action:'HOLD',allocation_pct:0,reason:`V29.6 HOLD: ${why}`};if(locked){counters.reentryBlocks++;re.stats.blocks++}else counters.blockedBelow56++
    }else if(locked&&score>=56){if(existing!==undefined)actions[existing]={...actions[existing],action:'HOLD',allocation_pct:0,reason:`V29.6 REENTRY-RESET: ${s} wartet auf ${lock.kind==='PLUS_10'?'Rücksetzer':'Erholung'} bis Score ${num(lock.rearmScore).toFixed(1)}. So entsteht kein direkter SELL→BUY-Gebührenloop.`};counters.reentryBlocks++;re.stats.blocks++}
  }
