...
export const PROFIT_EXIT_V297={
  ...
  profitFadeScoreDrop:3,
  profitFadeMaxScoreStep:0,
  profitFadeMaxChartStepPct:.15,
...
export function profitDecisionV297({entryScore=0,currentScore=0,chartMovePct=0,scoreDeltaThisScan=0,chartMoveLastScanPct=0}={}){
  ...
  const profitFade=delta<=-c.profitFadeScoreDrop&&scoreStep<=c.profitFadeMaxScoreStep&&chartStep<c.profitFadeMaxChartStepPct;
  if(profitFade)return{action:'SELL',reason:'profit_fade_lock',entryScore:entry,currentScore:current,delta,chartMovePct:chart,requiredDelta:0,strongRise,profitFade:true};
...
export function enforceProfitExitV297(plan,state={},storage=null,now=Date.now()){
  ...
  for(const p of arr(state?.positions)){
    ...
    if(d.action==='SELL'){
      ...
      const tier=d.reason==='profit_lock_5'?'5_PERCENT_LOCK':d.reason==='profit_fade_lock'?'PROFIT_FADE_LOCK':d.reason==='profit_3_5_score_4'?'3_5_PERCENT_PLUS_4':d.reason==='profit_2_score_7'?'2_PERCENT_PLUS_7':'0_8_PERCENT_PLUS_10';
      actions[i]={...a,symbol:s,action:'SELL',allocation_pct:0,confidence:.9,scoreExitV294:false,scoreExitV297:true,scoreExitKind:'PLUS_10',profitExitV297:true,profitExitTierV297:tier,profitFadeLockV2971:d.reason==='profit_fade_lock',scoreExitEntry:d.entryScore,scoreExitCurrent:d.currentScore,scoreExitDelta:d.delta,scoreExitChartMovePct:d.chartMovePct,requiredProfitScoreDeltaV297:d.requiredDelta,reason:`V29.7.1 GEWINN-SELL: ${s} Chart +${d.chartMovePct.toFixed(2)}%, Einstiegsscore ${d.entryScore.toFixed(1)} -> ${d.currentScore.toFixed(1)} (${d.delta>=0?'+':''}${d.delta.toFixed(1)}). ${tier==='5_PERCENT_LOCK'?'Ab +5% Gewinn wird gesichert, weil Score/Chart nicht mehr stark gemeinsam steigen.':tier==='PROFIT_FADE_LOCK'?'Vorhandener Gewinn wird gesichert, weil der Score seit Einstieg deutlich gefallen ist und aktuell weder Score noch Chart erneut stark beschleunigen.':`Erforderliche Scoreverbesserung ${d.requiredDelta.toFixed(1)} erreicht.`}`};
      const rearmScore=Math.max(56,d.currentScore-PROFIT_EXIT_V297.reentryPullbackPoints);re.locks[s]={at:now,kind:'PLUS_10',exitScore:d.currentScore,rearmScore,source:'V29.7.1_PROFIT_EXIT'};re.stats.locksCreated=num(re.stats.locksCreated)+1;counters.profitSells++;if(tier==='5_PERCENT_LOCK'||tier==='PROFIT_FADE_LOCK')counters.profitLocks++;if(tier==='PROFIT_FADE_LOCK')counters.profitFadeLocks++;continue;
    }
  ...
  plan.actions=actions;plan.summary=`${String(plan.summary||'').slice(0,130)} · V29.7.1 Gewinn: ${counters.profitSells} SELL · ${counters.profitFadeLocks} Fade-Lock · ${counters.strongRiseHolds} weiterlaufen · ${counters.oldProfitSellsSuppressed} alte Gewinn-SELL blockiert.`;return{plan,counters,positionScores:out.positionScores,audit:out.audit}
}
...
export class ProfitExitGuardV297{
  ...
  status(){const state=typeof this.getState==='function'?(this.getState()||{}):{},out=profitAdjustedPositionScoresV297(state,this.storage,typeof this.now==='function'?this.now():Date.now(),false);return{enabled:true,version:29.71,authoritativeProfitExit:true,thresholds:PROFIT_EXIT_V297,positionScores:out.positionScores,audit:out.audit,stats:out.stats,latest:this.latest?.counters||null,rule:'Gewinn-SELL gestaffelt: unter +0,8% HOLD. Ab +0,8% wird ein vorhandener Gewinn zusätzlich gesichert, wenn der Score seit Einstieg mindestens 3 Punkte gefallen ist und Score/Chart aktuell nicht erneut beschleunigen. Steigende Gewinner dürfen weiterlaufen. Ab +5% wird weiter grundsätzlich gesichert, außer Score und Chart steigen stark gemeinsam. -15 Schwäche-Exit bleibt separat unverändert.'}}
}
